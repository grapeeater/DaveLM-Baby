...
R=ROOT/'human_readiness_hr1_causal_aligned_seed87006_v1'; DEVICE='cuda'
 ...
 with torch.no_grad():
  for j in range(0,min(len(rows),128),32):
   ...
   for i,r in enumerate(rs):
    z=[2]+r['token_ids']+[3]; x[i,:len(z)-1]=torch.tensor(z[:-1],device=device); y[i,:len(z)-1]=torch.tensor(z[1:],device=device)
...
def main():
 ap=argparse.ArgumentParser(); ap.add_argument('--arm',choices=['factual','control'],required=True); ap.add_argument('--seed',type=int,default=87006); ap.add_argument('--output',required=True); ap.add_argument('--resume',action='store_true'); args=ap.parse_args(); assert args.seed==87006
 proto=json.loads((R/'HR1_CAUSAL_ALIGNED_PROTOCOL.json').read_text()); assert sha(Path(proto['parent_path']))==proto['parent_sha256']; assert torch.cuda.is_available()
 ...
 tr=[json.loads(x) for x in (R/'data'/'ENGLISH_TRAIN.jsonl').read_text(encoding='utf-8').splitlines()]; dv=[json.loads(x) for x in (R/'data'/'ENGLISH_DEV.jsonl').read_text(encoding='utf-8').splitlines()]; sched=json.loads((R/'data'/'ENGLISH_SCHEDULE.json').read_text())['batches']; bind=json.loads((R/proto['data']['binding_rehearsal']).read_text())['quartets']; bs=json.loads((R/'data'/'BINDING_SCHEDULE.json').read_text())['batches']; byid={d['doc_id']:d for q in bind for d in q['docs']}; rec={d['id']:d for d in tr}; opt=torch.optim.AdamW(m.parameters(),lr=5e-5,betas=(.9,.999),eps=1e-8,weight_decay=.05,amsgrad=False,foreach=False,fused=False); out=Path(args.output); out.mkdir(parents=True,exist_ok=True); start=1; devs={'0':dev_loss(m,dv,DEVICE)}
 if args.resume:
  st=torch.load(out/'restart.pt',map_location=DEVICE,weights_only=False); assert st['arm']==args.arm and st['parent_sha256']==proto['parent_sha256'] and st['protocol_sha256']==sha(R/'HR1_CAUSAL_ALIGNED_PROTOCOL.json') and st['schedule_sha256']==sha(R/'data'/'ENGLISH_SCHEDULE.json'); m.load_state_dict(st['model_state']); opt.load_state_dict(st['optimizer_state']); random.setstate(st['python_rng']); torch.set_rng_state(st['torch_rng'].cpu()); start=int(st['completed_update'])+1; devs=st.get('dev_loss',devs)
