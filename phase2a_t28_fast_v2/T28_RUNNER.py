"""Phase 2A T24: T18 two-phase recipe plus train+dev name-inventory unlikelihood.

Full-span answer CE through the base (T18). Plus mean -log(1-p) on all other
T3 train/dev name tokens at the same answer offset. Never loads T3 TEST.
"""
...
BUNDLE_DEFAULT = Path(r"C:\DaveLM-CADAVER\phase2a_t28_fast_v2")
...
def main():
    ...
    if args.seed not in (850001, 850002, 850003):
    ...
    def answer_span_ce_and_unl(row):
        ...
        st = y[0, -n:][1:]
        if sl.numel():
            tgt = sl.gather(1, st.unsqueeze(1)).squeeze(1)
            masked = sl.clone(); masked.scatter_(1, st.unsqueeze(1), float('-inf'))
            cont = torch.relu(torch.tensor(M, device=device) - tgt + masked.max(1).values).mean()
        else:
            cont = logits.new_zeros(())
        return ans, unl, cont
    ...
    def qa_update(indices):
        ...
        for b in range(0, len(indices), micro):
            ...
            for j in group:
                ...
                ans, unl, cont = answer_span_ce_and_unl(train_rows[j])
                loss = loss + (mar + LAMBDA_PTR * ptr + LAMBDA_ANS * ans + LAMBDA_UNL * unl + LAMBDA_CONT * cont) / len(group)
    ...
    for update in range(resume_from + 1, TOTAL_UPDATES + 1):
        ...
        atomic_torch(ckdir / f"checkpoint_{update:04d}.pt", {
            "schema": "baby_vnext_phase2a_t24_inventory_unlikelihood_pointer_lr3p75e5_1k_model_v1",
            "update": update,
            "seed": args.seed,
            "phase": phase,
            "model_state_dict": model.state_dict(),
        ...
        if rec["hard_stop"] or rec["dev_ce"] > 1.50:
            final_class, stop_code = "T24_HARD_STOP", "integrity_or_catastrophe"
            break
        if rec["dev_ce"] > 1.30:
            final_class, stop_code = "T24_LANGUAGE_REGRESSION", "language_ce_gt_1_30"
            break
