"""SF21: paired counterfactual representation consistency on SF20.
...
def paired_consistency(hidden, ids_in_batch, idx, pairs):
...
def main():
    ...
    provenance = {'parent_checkpoint': run['parent_checkpoint'], 'parent_checkpoint_sha256': run['parent_checkpoint_sha256'],
                  ...
                  'pair_manifest': sha(H / 'PAIR_MANIFEST.json'), 'lambda_consistency': LAMBDA_CONSISTENCY}
    ...
    for u in schedule[completed:]:
        ...
        if margin_loss is not None: rec.update(margin_loss=float(margin_loss.detach()), consistency_loss=float(consistency_loss.detach()), weighted_consistency=float((LAMBDA_CONSISTENCY*consistency_loss).detach()), mean_pair_cosine=float(pair_cosine.detach()))
