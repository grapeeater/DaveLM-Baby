"""Phase 2A T19: T18 two-phase recipe plus suffix-weighted answer-span CE.

Phase A trains 4-11 for 750; after U750 revert/freeze 8-11 at Phase1G and train 4-7 for 250.
QA loss = margin + lambda_ptr * pointer + lambda_ans * suffix_weighted_answer_ce.
First answer token weight 1.0; remaining name tokens + period + EOS weight 3.0.
Never loads T3 TEST.
"""
...
BUNDLE_DEFAULT = Path(r"C:\DaveLM-CADAVER\phase2a_t19_suffix_answerce_lr3p75e5_1k_v1")
...
def main():
    ...
    if args.seed not in (780001, 780002, 780003):
    ...
    def answer_span_ce(row):
        ...
        logits, _ = model(x)
        n = len(target)
        tok_ce = F.cross_entropy(logits[0, -n:, :], y[0, -n:], reduction="none")
        w = torch.ones(n, device=device)
        if n > 1:
            w[1:] = ANSWER_SUFFIX_W
        return (tok_ce * w).sum() / w.sum()
    ...
    for update in range(resume_from + 1, TOTAL_UPDATES + 1):
        ...
        atomic_torch(ckdir / f"checkpoint_{update:04d}.pt", {
            "schema": "baby_vnext_phase2a_t19_suffix_answerce_pointer_lr3p75e5_1k_model_v1",
            "update": update,
            "seed": args.seed,
            "phase": phase,
            "model_state_dict": model.state_dict(),
        ...
        if rec["hard_stop"] or rec["dev_ce"] > 1.50:
            final_class, stop_code = "T19_HARD_STOP", "integrity_or_catastrophe"
            break
        if rec["dev_ce"] > 1.30:
            final_class, stop_code = "T19_LANGUAGE_REGRESSION", "language_ce_gt_1_30"
            break
