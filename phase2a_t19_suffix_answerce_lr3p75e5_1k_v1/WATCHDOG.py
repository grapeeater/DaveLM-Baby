"""Detached T19 sequential watchdog. Never loads T3 TEST."""
...
RUNNER = Path(r"C:\DaveLM-CADAVER\phase2a_t19_suffix_answerce_lr3p75e5_1k_v1\T19_RUNNER.py")
BUNDLE = Path(r"C:\DaveLM-CADAVER\phase2a_t19_suffix_answerce_lr3p75e5_1k_v1")
ROOT = Path(r"C:\DaveLM-CADAVER")
SEEDS = (780001, 780002, 780003)
HEARTBEAT = BUNDLE / "WATCHDOG_HEARTBEAT.json"
LOG = ROOT / "phase2a_t19_suffix_answerce_lr3p75e5_1k_v1_watchdog.log"
POLL = 20
STUDY = "BABY_VNEXT_PHASE2A_T19_SUFFIX_ANSWERCE_LR3P75E5_1K_V1"
...
def live_cmds():
    ...
    return [ln.strip() for ln in raw.splitlines()
            if "T19_RUNNER.py" in ln or ("t19_suffix_answerce" in ln.lower() and "WATCHDOG" in ln)]


def out_dir(seed):
    return ROOT / f"phase2a_t19_suffix_answerce_lr3p75e5_1k_v1_run_seed{seed}"
...
def owner_live():
    return any("T19_RUNNER.py" in c for c in live_cmds())


def run_seed(seed, resume):
    out = out_dir(seed)
    log_path = ROOT / f"phase2a_t19_suffix_answerce_lr3p75e5_1k_v1_train_seed{seed}.log"
    env = os.environ.copy()
...
def classify_study(results):
    ...
    if any(c == "T19_HARD_STOP" for c in classes):
        return "T19_HARD_STOP"
    if classes.count("T19_FULL_SUCCESS") >= 2:
        return "T19_FULL_SUCCESS"
    if sum(c in ("T19_FULL_SUCCESS", "T19_REPRESENTATION_SUCCESS_OUTPUT_FAIL") for c in classes) >= 2:
        return "T19_REPRESENTATION_SUCCESS_OUTPUT_FAIL"
    if classes.count("T19_LANGUAGE_REGRESSION") >= 2:
        return "T19_LANGUAGE_REGRESSION"
    if classes.count("T19_SHORTCUT_FAILURE") >= 2:
        return "T19_SHORTCUT_FAILURE"
    return "T19_FAIL_NO_REPRESENTATION"
