...
def verify_bundle(bundle):
 ...
 assert receipt['status']=='FULLY_EXECUTABLE_PRETRAINING_FREEZE_COMPLETE'
...
def run(bundle,arm,seed,parent,out,resume=False,mock=False,preflight_only=False):
 ...
 if preflight_only:
  return {'status':'PREEXECUTION_PASS_PARENT_LOADING_BOUNDARY','bundle_verified':True,'parent_hash_verified':True,'checkpoint_loaded':False,'optimizer_created':False,'optimizer_updates':0,'inference':False,'confirmation_locked':True,'sacred_access':False}
 ...
 for u in schedule['updates'][start:]:
  ...
  if u['kind']=='english':
   rs=[byid[x] for x in u['arms'][arm]['item_ids']]; x,labels=mask_english(rs,u['shared_pad_to_length']); x=x.to(device); labels=labels.to(device); logits=model.base_model(x); loss=F.cross_entropy(logits.reshape(-1,logits.size(-1)),labels.reshape(-1),ignore_index=-100)
  else:
