"""Independent full-trajectory SF9 runs: widen Baby's factual-selection rule across surface
forms and competing-information orders while preserving SF8's lambda=0.25 margin recipe and all
retention machinery. Parent = a distinct SF8 lambda=0.25 checkpoint (fresh optimizer, no LR change).
"""
...
def verify(sealed=True):
    if sealed:
        ...
        assert receipt['status'] == 'SF9_PROSPECTIVE_PREFLIGHT_PASS'
...
def load_inputs():
    ...
    for n, u in enumerate(schedule, 1):
        ...
        if u['kind'] == 'english':
            ei += 1
            assert len(u['ids']) == 144 and Counter(u['ids']) == Counter({k: 1 for k in idx})
            x, y = E.pad_batch([idx[i] for i in u['ids']], u['pad'])
            assert x.shape == y.shape == (144, u['pad']) and u['pad'] <= 256
            assert int((y != -100).sum()) == 144 * 5
...
def dev_surface_gate(agg):
    return agg['correct'] >= 15 and agg['exact'] >= 13
...
def dev_order_gate(agg):
    ...
    return agg['exact'] >= 13 and fact >= 7 and copy >= 7
...
def main():
    ...
    if a.mode == 'preload':
        print('SF9_PRE_PARENT_LOAD_PASS', a.seed, flush=True); return
    ...
    # Load SF8 parent checkpoint weights (fresh optimizer, no optimizer state carried).
    ...
    if completed == 0:
        ...
        # Parent integrity: the SF8 checkpoint must already satisfy acquisition + retention.
        assert retention_acq(ret), ret
        assert all(v['gate'] for v in c['binding'].values())
        assert d3['mean_combined_name_probability'] <= .01
