"""Rebuilt Phase 2A Treatment 3 runner.

Pointer + T2 native margin on a frozen disjoint corpus. Binding params frozen.
TEST is not loaded unless --score-test after a valid DEV terminal commit.
"""
...
BUNDLE_DEFAULT = Path(r"C:\DaveLM-CADAVER\phase2a_t3_rebuilt_study_v1")
...
def classify(rec: dict, u0_ce: float) -> str:
    ...
    if not rec["binding_intact"] or not math.isfinite(rec["dev_ce"]):
        return "T3_HARD_STOP"
...
def main():
    ...
    if args.seed not in (620001, 620002, 620003):
    ...
    train_rows = load_jsonl(bundle / "data" / "qa_train.jsonl")
    dev_rows = load_jsonl(bundle / "data" / "qa_dev.jsonl")
    acq_rows = json.loads((bundle / "data" / "acq16.json").read_text(encoding="utf-8"))
    panels = json.loads((bundle / "data" / "panels.json").read_text(encoding="utf-8"))
    ...
    seal = json.loads((bundle / "data" / "TEST_SEAL.json").read_text(encoding="utf-8"))
    ...
    for update in range(resume_from + 1, 501):
        ...
        if update not in (100, 300, 500):
            continue
        rec = run_full(update)
        rec["hard_stop"] = (not rec["binding_intact"]) or (not math.isfinite(rec["dev_ce"]))
        ...
        atomic_torch(ckdir / f"checkpoint_{update:04d}.pt", {
            "schema": "baby_vnext_phase2a_t3_rebuilt_model_v1",
            "update": update,
            "seed": args.seed,
            "model_state_dict": model.state_dict(),
        })
        if rec["hard_stop"] or rec["dev_ce"] > 1.50:
            final_class, stop_code = "T3_HARD_STOP", "integrity_or_catastrophe"
            break
        if update == 500:
