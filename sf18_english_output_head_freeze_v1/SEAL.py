...
def sha(p):
 ...
 return h.hexdigest()
def main():
 assert json.loads((H/'PREFLIGHT.json').read_text())['status']=='SF18_PROSPECTIVE_PREFLIGHT_PASS'
 ...
 rec={'status':'SF18_PROSPECTIVE_PREFLIGHT_PASS','study':'SF18_ENGLISH_OUTPUT_HEAD_FREEZE_V1','manifest_sha256':mh,'sealed_utc':datetime.now(timezone.utc).isoformat(),'sole_scientific_change':'untied language head frozen during English updates only','checkpoint_loaded':False,'optimizer_created':False,'updates':0,'final_accessed':False,'sacred_accessed':False}
