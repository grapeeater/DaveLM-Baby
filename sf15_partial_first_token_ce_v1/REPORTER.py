"""Frozen SF15 aggregation and prospective classification."""
import hashlib,json
...
def classify(rows):
 ...
 if sum(r['gates']['endpoint_pass'] for r in rows)>=2: return 'PARTIAL_FIRST_TOKEN_CE_SUPPORTED'
 ...
 if d3_tradeoff>=2 or sf14_like>=2: return 'PARTIAL_FIRST_TOKEN_CE_INSUFFICIENT'
...
def main():
 ...
 if cls=='PARTIAL_FIRST_TOKEN_CE_SUPPORTED': lines.append('A 0.25 first-answer-token CE weight was sufficient under SF15 to preserve D3 while recovering exact factual generation/retention and maintaining tested language, binding, and widening behavior. This does not establish optimality, sole causation, or a general scaling law.')
 elif cls=='PARTIAL_FIRST_TOKEN_CE_INSUFFICIENT': lines.append('The 0.25 first-answer-token CE treatment reproduced at least two branches of the preregistered D3-versus-exact-generation tradeoff. This identifies the observed tradeoff under SF15 without establishing CE as its sole cause.')
 elif cls=='RETENTION_REGRESSION': lines.append('SF15 failed a frozen TRAIN16, language, or binding retention requirement; safety failure takes precedence.')
 else: lines.append('The valid SF15 branches did not produce a decisive preregistered pattern.')
