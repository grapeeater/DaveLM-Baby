    ...
    for partition in ('primary', 'confirmation'):
        ...
        for f in chosen:
            for i, (n, v, d) in enumerate(facts_for(f, 0)):
                prompt = (f'{n} {v} the {d} and then went home.' if partition == 'primary'
                          else f'After {n} {v} the {d}, {n} went home.')
                ids = tok.encode(prompt, add_special_tokens=False).ids
                assert tok.decode(ids, skip_special_tokens=False) == prompt
                if partition == 'primary':
                    m = re.fullmatch(r'(Alex|Mia|Nora|Owen) (found|carried) the (\w+ \w+) and then went home\.', prompt)
                    assert m and m.groups() == (n, v, d)
                else:
                    m = re.fullmatch(r'After (Alex|Mia|Nora|Owen) (found|carried) the (\w+ \w+), (Alex|Mia|Nora|Owen) went home\.', prompt)
                    assert m and m.groups()[:3] == (n, v, d) and m.group(1) == m.group(4)
                natural.append({'id': f"{f['family_id']}:natural:{i}", 'partition': partition,
                                'family_id': f['family_id'], 'prompt': prompt, 'prompt_token_ids': ids,
                                'semantic_tuple': [n, v, d], 'renderer': partition,
...
def uniqueness(rows, transfers, natural):
    ...
    primary = {norm(r['prompt']) for r in natural if r['partition'] == 'primary'}
    confirmation = {norm(r['prompt']) for r in natural if r['partition'] == 'confirmation'}
    return {'factual_controlled_count': len(factual), 'unique_factual_prompts': len(counts),
            ...
            'naturalistic_by_partition': {'primary': len(primary), 'confirmation': len(confirmation)},
            'naturalistic_cross_holdout_intersection': sorted(primary & confirmation)}
...
def main():
    ...
    for r in natural:
        expected = tuple(r['semantic_tuple'])
        assert independent.parse_natural(r['prompt'], r['renderer']) == expected
    ...
    if check_unique['naturalistic_cross_holdout_intersection']:
        issues.append('Primary/Confirmation naturalistic prompt intersection')
    if check_unique['naturalistic_by_partition'] != {'primary': 16, 'confirmation': 16}:
        issues.append('Naturalistic count invariant failed')
