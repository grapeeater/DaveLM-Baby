...
R=ROOT/'human_readiness_hr1_seed87004_v3'; DEVICE='cuda'
import torch.nn as nn
def basis(u):
 un=u/torch.linalg.vector_norm(u); p=int(torch.argmax(torch.abs(un))); e=torch.zeros_like(un); e[p]=1. if un[p]>=0 else -1.; w=un-e; H=torch.eye(un.numel(),device=u.device,dtype=u.dtype)-2*torch.outer(w,w,device=u.device)/torch.dot(w,w); return H[:,[i for i in range(un.numel()) if i!=p]]
