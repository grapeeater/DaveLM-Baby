"""Detached T15 sequential watchdog. Never loads T3 TEST."""
...
RUNNER = Path(r"C:\DaveLM-CADAVER\phase2a_t15_latebase_blocks4to7_lr3p75e5_750_v1\T15_RUNNER.py")
BUNDLE = Path(r"C:\DaveLM-CADAVER\phase2a_t15_latebase_blocks4to7_lr3p75e5_750_v1")
ROOT = Path(r"C:\DaveLM-CADAVER")
SEEDS = (740001, 740002, 740003)
HEARTBEAT = BUNDLE / "WATCHDOG_HEARTBEAT.json"
LOG = ROOT / "phase2a_t15_latebase_blocks4to7_lr3p75e5_750_v1_watchdog.log"
POLL = 20
...
def live_cmds():
    ...
    return [ln.strip() for ln in raw.splitlines()
            if "T15_RUNNER.py" in ln or ("t15_latebase_blocks4to7" in ln.lower() and "WATCHDOG" in ln)]


def out_dir(seed):
    return ROOT / f"phase2a_t15_latebase_blocks4to7_lr3p75e5_750_v1_run_seed{seed}"
...
def owner_live():
    return any("T15_RUNNER.py" in c for c in live_cmds())


def run_seed(seed, resume):
    out = out_dir(seed)
    log_path = ROOT / f"phase2a_t15_latebase_blocks4to7_lr3p75e5_750_v1_train_seed{seed}.log"
    env = os.environ.copy()
...
def classify_study(results):
    ...
    if any(c == "T15_HARD_STOP" for c in classes):
        return "T15_HARD_STOP"
    if classes.count("T15_FULL_SUCCESS") >= 2:
        return "T15_FULL_SUCCESS"
    if sum(c in ("T15_FULL_SUCCESS", "T15_REPRESENTATION_SUCCESS_OUTPUT_FAIL") for c in classes) >= 2:
        return "T15_REPRESENTATION_SUCCESS_OUTPUT_FAIL"
    if classes.count("T15_LANGUAGE_REGRESSION") >= 2:
        return "T15_LANGUAGE_REGRESSION"
    if classes.count("T15_SHORTCUT_FAILURE") >= 2:
        return "T15_SHORTCUT_FAILURE"
    return "T15_FAIL_NO_REPRESENTATION"
...
def main():
    ...
    write_json(BUNDLE / "RUN_LEDGER.json", {
        ...
        "study": "BABY_VNEXT_PHASE2A_T15_LATEBASE_BLOCKS4TO7_LR3P75E5_750_V1",
        ...
        "scope": {"trainable": 21163264, "frozen": 40357121},
