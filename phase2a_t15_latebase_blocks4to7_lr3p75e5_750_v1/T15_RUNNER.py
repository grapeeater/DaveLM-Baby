"""Phase 2A T15: T14 recipe with blocks 8-11 frozen at Phase1G; train blocks 4-7 + embed/head.
...
BUNDLE_DEFAULT = Path(r"C:\DaveLM-CADAVER\phase2a_t15_latebase_blocks4to7_lr3p75e5_750_v1")
...
def freeze_early(name: str) -> bool:
    if any(name.startswith(f"base_model.blocks.{i}.") for i in range(4)):
        return True
    if any(name.startswith(f"base_model.blocks.{i}.") for i in range(8, 12)):
        return True
    return False
...
def set_scope(model):
    for n, p in model.named_parameters():
        train = n.startswith("base_model.") and not freeze_early(n)
...
def main():
    ...
    if args.seed not in (740001, 740002, 740003):
    ...
    if len(kinds) != 750 or kinds.count("lang") != 75 or kinds.count("qa") != 675:
        raise RuntimeError(f"kinds freeze mismatch len={len(kinds)}")
    if len(qa_sched) != 675 or any(len(b) != 32 for b in qa_sched):
        raise RuntimeError(f"qa schedule freeze mismatch len={len(qa_sched)}")
    if len(rehear) != 75 or any(len(x) != 64 for x in rehear):
    ...
    if scope != {"trainable": 21163264, "frozen": 40357121}:
    ...
    if args.resume:
        ...
        write_json(out / "STATUS.json", {
            ...
            "resumed": True, "parent_sha256": PARENT_SHA,
        })
    else:
    ...
    def qa_update(indices):
        ...
        for b in range(0, len(indices), micro):
            ...
            for j in group:
                ...
                loss = loss + (mar + LAMBDA_PTR * ptr) / len(group)
    ...
    for update in range(resume_from + 1, 751):
        kind = kinds[update - 1]
        model.train()
        ...
        bad = [n for n, p in model.named_parameters()
               if p.grad is not None and (freeze_early(n) or not n.startswith("base_model."))]
        if bad:
        ...
        if update not in (100, 300, 500, 750):
            continue
        rec = run_full(update)
        ...
        atomic_torch(ckdir / f"checkpoint_{update:04d}.pt", {
            "schema": "baby_vnext_phase2a_t15_latebase_blocks4to7_pointer_lr3p75e5_750_model_v1",
            "update": update,
            "seed": args.seed,
            "model_state_dict": model.state_dict(),
        })
        if rec["hard_stop"] or rec["dev_ce"] > 1.50:
            final_class, stop_code = "T15_HARD_STOP", "integrity_or_catastrophe"
            break
        if rec["dev_ce"] > 1.30:
            final_class, stop_code = "T15_LANGUAGE_REGRESSION", "language_ce_gt_1_30"
            break
        if update == 750:
