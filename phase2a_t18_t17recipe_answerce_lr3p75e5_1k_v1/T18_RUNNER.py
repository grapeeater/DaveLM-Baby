"""Phase 2A T18: T17 two-phase recipe plus answer-span teacher-forced CE on QA updates.

Phase A trains 4-11 for 750; after U750 revert/freeze 8-11 at Phase1G and train 4-7 for 250.
QA loss = margin + lambda_ptr * pointer + lambda_ans * answer_span_ce.
Never loads T3 TEST.
"""
...
BUNDLE_DEFAULT = Path(r"C:\DaveLM-CADAVER\phase2a_t18_t17recipe_answerce_lr3p75e5_1k_v1")
...
def main():
    ...
    if args.seed not in (770001, 770002, 770003):
    ...
    for update in range(resume_from + 1, TOTAL_UPDATES + 1):
        ...
        if rec["hard_stop"] or rec["dev_ce"] > 1.50:
            final_class, stop_code = "T18_HARD_STOP", "integrity_or_catastrophe"
            break
        if rec["dev_ce"] > 1.30:
            final_class, stop_code = "T18_LANGUAGE_REGRESSION", "language_ce_gt_1_30"
            break
