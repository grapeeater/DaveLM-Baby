"""Detached T18 sequential watchdog. Never loads T3 TEST."""
...
RUNNER = Path(r"C:\DaveLM-CADAVER\phase2a_t18_t17recipe_answerce_lr3p75e5_1k_v1\T18_RUNNER.py")
BUNDLE = Path(r"C:\DaveLM-CADAVER\phase2a_t18_t17recipe_answerce_lr3p75e5_1k_v1")
ROOT = Path(r"C:\DaveLM-CADAVER")
SEEDS = (770001, 770002, 770003)
HEARTBEAT = BUNDLE / "WATCHDOG_HEARTBEAT.json"
LOG = ROOT / "phase2a_t18_t17recipe_answerce_lr3p75e5_1k_v1_watchdog.log"
POLL = 20
STUDY = "BABY_VNEXT_PHASE2A_T18_T17RECIPE_ANSWERCE_LR3P75E5_1K_V1"
...
def live_cmds():
    ...
    return [ln.strip() for ln in raw.splitlines()
            if "T18_RUNNER.py" in ln or ("t18_t17recipe_answerce" in ln.lower() and "WATCHDOG" in ln)]


def out_dir(seed):
    return ROOT / f"phase2a_t18_t17recipe_answerce_lr3p75e5_1k_v1_run_seed{seed}"
...
def owner_live():
    return any("T18_RUNNER.py" in c for c in live_cmds())


def run_seed(seed, resume):
    out = out_dir(seed)
    log_path = ROOT / f"phase2a_t18_t17recipe_answerce_lr3p75e5_1k_v1_train_seed{seed}.log"
    env = os.environ.copy()
...
def classify_study(results):
    ...
    if any(c == "T18_HARD_STOP" for c in classes):
        return "T18_HARD_STOP"
    if classes.count("T18_FULL_SUCCESS") >= 2:
        return "T18_FULL_SUCCESS"
    if sum(c in ("T18_FULL_SUCCESS", "T18_REPRESENTATION_SUCCESS_OUTPUT_FAIL") for c in classes) >= 2:
        return "T18_REPRESENTATION_SUCCESS_OUTPUT_FAIL"
    if classes.count("T18_LANGUAGE_REGRESSION") >= 2:
        return "T18_LANGUAGE_REGRESSION"
    if classes.count("T18_SHORTCUT_FAILURE") >= 2:
        return "T18_SHORTCUT_FAILURE"
    return "T18_FAIL_NO_REPRESENTATION"
