"""Detached T24 sequential watchdog. Never loads T3 TEST."""
...
RUNNER = Path(r"C:\DaveLM-CADAVER\phase2a_t24_inventory_unlikelihood_lr3p75e5_1k_v1\T24_RUNNER.py")
BUNDLE = Path(r"C:\DaveLM-CADAVER\phase2a_t24_inventory_unlikelihood_lr3p75e5_1k_v1")
ROOT = Path(r"C:\DaveLM-CADAVER")
SEEDS = (830001, 830002, 830003)
HEARTBEAT = BUNDLE / "WATCHDOG_HEARTBEAT.json"
LOG = ROOT / "phase2a_t24_inventory_unlikelihood_lr3p75e5_1k_v1_watchdog.log"
POLL = 20
STUDY = "BABY_VNEXT_PHASE2A_T24_INVENTORY_UNLIKELIHOOD_LR3P75E5_1K_V1"
...
def live_cmds():
    ...
    return [ln.strip() for ln in raw.splitlines()
            if "T24_RUNNER.py" in ln or ("t24_inventory_unlikelihood" in ln.lower() and "WATCHDOG" in ln)]


def out_dir(seed):
    return ROOT / f"phase2a_t24_inventory_unlikelihood_lr3p75e5_1k_v1_run_seed{seed}"
...
def owner_live():
    return any("T24_RUNNER.py" in c for c in live_cmds())


def run_seed(seed, resume):
    out = out_dir(seed)
    log_path = ROOT / f"phase2a_t24_inventory_unlikelihood_lr3p75e5_1k_v1_train_seed{seed}.log"
    env = os.environ.copy()
...
def classify_study(results):
    ...
    if any(c == "T24_HARD_STOP" for c in classes):
        return "T24_HARD_STOP"
    if classes.count("T24_FULL_SUCCESS") >= 2:
        return "T24_FULL_SUCCESS"
    if sum(c in ("T24_FULL_SUCCESS", "T24_REPRESENTATION_SUCCESS_OUTPUT_FAIL") for c in classes) >= 2:
        return "T24_REPRESENTATION_SUCCESS_OUTPUT_FAIL"
    if classes.count("T24_LANGUAGE_REGRESSION") >= 2:
        return "T24_LANGUAGE_REGRESSION"
    if classes.count("T24_SHORTCUT_FAILURE") >= 2:
        return "T24_SHORTCUT_FAILURE"
    return "T24_FAIL_NO_REPRESENTATION"
...
def main():
    ...
    write_json(BUNDLE / "RUN_LEDGER.json", {
        ...
        "scope": {"phase_a_updates": "1-750", "phase_a_trainable": 40849664, "phase_b_after": 750, "phase_b_trainable": 21163264},
        "pointer_keys": "fact_clause_name_through_period_token_18",
        "language_mix": "9_qa_1_lang",
        "lr": 3.75e-5,
        "lambda_ans": 1.0,
        "lambda_unl": 1.0,
        "answer_ce": "t18_fullspan_through_base_plus_traindev_name_inventory_unlikelihood",
        "total_updates": 1000,
    })
    while True:
        ...
        if all(complete(s) for s in SEEDS):
            ...
            write_json(BUNDLE / "STUDY_STATUS.json", {
                "study": STUDY, "study_classification": study, "runs": results, "test_opened": False,
            })
