"""Phase 2A T5: freeze blocks 0-3; train late base + binding. Never loads T3 TEST."""
...
BUNDLE_DEFAULT = Path(r"C:\DaveLM-CADAVER\phase2a_t5_latebase_binding_v1")
...
def main():
    ...
    def qa_update(indices):
        ...
        for b in range(0, len(indices), micro):
            ...
            for j in group:
                ...
                loss = loss + (mar + LAMBDA_PTR * ptr + LAMBDA_LOC * loc) / len(group)
    ...
    for update in range(resume_from + 1, 501):
        ...
        atomic_torch(ckdir / f"checkpoint_{update:04d}.pt", {
            "schema": "baby_vnext_phase2a_t5_latebase_binding_model_v1",
            "update": update, "seed": args.seed,
            "model_state_dict": model.state_dict(),
        })
        if rec["hard_stop"]:
            final_class, stop_code = "T5_HARD_STOP", "integrity_or_catastrophe"
            break
        if rec["dev_ce"] > 1.30:
            final_class, stop_code = "T5_LANGUAGE_REGRESSION", "language_ce_gt_1_30"
            break
        if update == 500:
