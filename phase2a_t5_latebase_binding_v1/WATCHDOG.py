"""Detached T5 sequential watchdog. Never loads T3 TEST."""
...
RUNNER = Path(r"C:\DaveLM-CADAVER\phase2a_t5_latebase_binding_v1\T5_RUNNER.py")
BUNDLE = Path(r"C:\DaveLM-CADAVER\phase2a_t5_latebase_binding_v1")
ROOT = Path(r"C:\DaveLM-CADAVER")
SEEDS = (640001, 640002, 640003)
HEARTBEAT = BUNDLE / "WATCHDOG_HEARTBEAT.json"
LOG = ROOT / "phase2a_t5_latebase_binding_v1_watchdog.log"
POLL = 20
...
def live_cmds():
    ...
    return [ln.strip() for ln in raw.splitlines()
            if "T5_RUNNER.py" in ln or ("t5_latebase" in ln.lower() and "WATCHDOG" in ln)]


def out_dir(seed):
    return ROOT / f"phase2a_t5_latebase_binding_v1_run_seed{seed}"
...
def owner_live():
    return any("T5_RUNNER.py" in c for c in live_cmds())


def run_seed(seed, resume):
    out = out_dir(seed)
    log_path = ROOT / f"phase2a_t5_latebase_binding_v1_train_seed{seed}.log"
    env = os.environ.copy()
...
def classify_study(results):
    ...
    if any(c == "T5_HARD_STOP" for c in classes):
        return "T5_HARD_STOP"
    if classes.count("T5_JOINT_SUCCESS") >= 2:
        return "T5_JOINT_SUCCESS"
    if sum(c in ("T5_JOINT_SUCCESS", "T5_ROUTING_SUCCESS_OUTPUT_FAIL") for c in classes) >= 2:
        return "T5_ROUTING_SUCCESS_OUTPUT_FAIL"
    if classes.count("T5_LANGUAGE_REGRESSION") >= 2:
        return "T5_LANGUAGE_REGRESSION"
    return "T5_FAIL_NO_ROUTING"
...
def main():
    ...
    write_json(BUNDLE / "RUN_LEDGER.json", {
        ...
        "study": "BABY_VNEXT_PHASE2A_T5_LATEBASE_BINDING_V1",
        ...
        "scope": {"trainable": 41833985, "frozen": 19686400},
    })
    while True:
