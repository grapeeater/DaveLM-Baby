"""Independent full-trajectory SF6 runs. Pinned SF2 objectives/evaluators, one LR variable."""
...
def verify(sealed=True):
    ...
    for path,h in read(H/'EXTERNAL_INPUTS.json').items(): assert sha(path)==h,path
...
def load_inputs():
    schedule=read(H/'SCHEDULE.json'); lrs=read(H/'LR_SCHEDULE.json')
    ...
    assert len(schedule)==len(lrs)==200 and len(train)==len(idx)==16
    assert len(pool)==80 and sum(len(q['docs']) for q in pool)==320
    ei=0; late=0
    for n,(u,l) in enumerate(zip(schedule,lrs),1):
        assert u['update']==l['update']==n and u['kind']==l['kind']
        assert u['kind']==('binding' if n%10==0 else 'english')
        assert l['control']==5e-5
        if u['kind']=='english':
            ei+=1
        ...
        expected=5e-5*(90-late)/89 if n>100 and u['kind']=='english' else 5e-5
        assert l['anneal']==expected
    assert ei==180 and late==90 and lrs[100]['anneal']==5e-5 and lrs[198]['anneal']==0 and lrs[199]['anneal']==5e-5
    return schedule,lrs,train,idx,pool,kl,oldproto
...
def main():
    ...
    ap.add_argument('--arm',choices=['control','anneal'],required=True)
    ...
    schedule,lrs,train,idx,pool,kl,oldproto=load_inputs()
    rt.configure_runtime(a.seed)
    if a.mode=='preload':
        print('SF6_PRE_PARENT_LOAD_PASS',a.seed,a.arm,flush=True); return
    ...
    provenance={'parent':p['parent_sha256'],'protocol':sha(H/'PROTOCOL.json'),
                ...
                'lr_schedule':sha(H/'LR_SCHEDULE.json'),'controller':sha(Path(__file__)),
    ...
    for u in schedule[completed:]:
        ...
        number=u['update']
        rec={'update':number,'kind':u['kind'],'lr':lr,'loss':float(loss.detach()),'grad_norm':float(norm)}
