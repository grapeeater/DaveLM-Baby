...
def parse_natural(prompt, renderer):
    if renderer == 'primary':
        m = re.fullmatch(r'(Alex|Mia|Nora|Owen) (found|carried) the (blue|green|red|yellow) (ball|book|box|car|fish|hat|kite|toy) and then went home\.', prompt)
        assert m
        return m.group(1), m.group(2), m.group(3) + ' ' + m.group(4)
    assert renderer == 'confirmation'
    m = re.fullmatch(r'After (Alex|Mia|Nora|Owen) (found|carried) the (blue|green|red|yellow) (ball|book|box|car|fish|hat|kite|toy), (Alex|Mia|Nora|Owen) went home\.', prompt)
    assert m and m.group(1) == m.group(5)
    return m.group(1), m.group(2), m.group(3) + ' ' + m.group(4)
