"""Detached T29 sequential watchdog. Never loads T3 TEST."""
...
RUNNER = Path(r"C:\DaveLM-CADAVER\phase2a_t29_disambiguation_fork_v1\T29_RUNNER.py")
BUNDLE = Path(r"C:\DaveLM-CADAVER\phase2a_t29_disambiguation_fork_v1")
ROOT = Path(r"C:\DaveLM-CADAVER")
SEEDS = (860001, 860002, 860003)
HEARTBEAT = BUNDLE / "WATCHDOG_HEARTBEAT.json"
LOG = ROOT / "phase2a_t29_disambiguation_fork_v1_watchdog.log"
POLL = 20
STUDY = "BABY_VNEXT_PHASE2A_T29_DISAMBIGUATION_FORK_LR3P75E5_1K_V1"
...
def live_cmds():
    ...
    return [ln.strip() for ln in raw.splitlines()
            if "T29_RUNNER.py" in ln or ("t29_disambiguation_fork" in ln.lower() and "WATCHDOG" in ln)]


def out_dir(seed):
    return ROOT / f"phase2a_t29_disambiguation_fork_v1_run_seed{seed}"
...
def owner_live():
    return any("T29_RUNNER.py" in c for c in live_cmds())


def run_seed(seed, resume):
    out = out_dir(seed)
    log_path = ROOT / f"phase2a_t29_disambiguation_fork_v1_train_seed{seed}.log"
    env = os.environ.copy()
...
def classify_study(results):
    ...
    if any(c == "T29_HARD_STOP" for c in classes):
        return "T29_HARD_STOP"
    if classes.count("T29_FULL_SUCCESS") >= 2:
        return "T29_FULL_SUCCESS"
    if sum(c in ("T29_FULL_SUCCESS", "T29_REPRESENTATION_SUCCESS_OUTPUT_FAIL") for c in classes) >= 2:
        return "T29_REPRESENTATION_SUCCESS_OUTPUT_FAIL"
    if classes.count("T29_LANGUAGE_REGRESSION") >= 2:
        return "T29_LANGUAGE_REGRESSION"
    if classes.count("T29_SHORTCUT_FAILURE") >= 2:
