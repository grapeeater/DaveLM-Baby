"""Phase 2A T29: T18/T24 two-phase recipe plus TRAIN identity-disambiguation fork hinge.

Inherited T24 stack (full-span answer CE, pointer, candidate margin, inventory
unlikelihood, 9:1 language). T28 suffix-vocab margin is OFF. New term L_fork
fires only at the first TRAIN-identity unique-commit token. Never loads T3 TEST.
"""
...
BUNDLE_DEFAULT = Path(r"C:\DaveLM-CADAVER\phase2a_t29_disambiguation_fork_v1")
...
LAMBDA_FORK = 0.5
FORK_MARGIN = 1.0
AUTHORIZED_SEEDS = (860001, 860002, 860003)
CTX = 256
EOS = 3
...
def classify(rec: dict, u0_ce: float) -> str:
    ...
    if rep and max(sc.values()) >= (ptr / rec["dev"]["n"]) - 0.02:
        return "T29_SHORTCUT_FAILURE"
    nat = (rec["dev"]["native_forced_choice"] >= 96
           and rec["dev_reversals"]["native"] >= 48
           and rec["dev"]["exact_answer_eos"] >= 80)
    acq = rec["acq16_pass"]
    if not lang_ok:
        return "T29_LANGUAGE_REGRESSION"
    if not rep:
        return "T29_FAIL_NO_REPRESENTATION"
    if not nat or not acq:
        return "T29_REPRESENTATION_SUCCESS_OUTPUT_FAIL"
    return "T29_FULL_SUCCESS"
...
def main():
    ...
    frozen_forks = json.loads((bundle / "data" / "train_fork_table.json").read_text(encoding="utf-8"))
    if frozen_forks.get("never_used_for_fork_construction") != ["qa_dev.jsonl", "qa_test.jsonl", "DEV fail-modes"]:
        raise RuntimeError("fork table provenance mismatch")
    for n in train_names:
        a, b = train_forks[n], frozen_forks["forks"][n]
        if (a["has_fork"] != b["has_fork"] or a["fork_index"] != b["fork_index"]
                or a["gold_token"] != b["gold_token"] or a["competitor_tokens"] != b["competitor_tokens"]):
            raise RuntimeError(f"fork table freeze mismatch for {n}")
    ...
    def answer_span_ce_and_unl(row):
        ...
        sl = logits[0, -n:, :]
        st = y[0, -n:]
        spec = train_forks.get(gold_name) or {}
        if spec.get("has_fork") and spec.get("competitor_tokens"):
            k = int(spec["fork_index"])
            gold_t = int(st[k].item())
            if gold_t != int(spec["gold_token"]):
                raise RuntimeError(f"fork gold token mismatch {gold_name}")
            row_logits = sl[k]
            comps = torch.tensor(spec["competitor_tokens"], device=device, dtype=torch.long)
            fork = torch.relu(row_logits.new_tensor(FORK_MARGIN) - row_logits[gold_t] + row_logits.index_select(0, comps).max())
        else:
            fork = logits.new_zeros(())
        return ans, unl, fork
    ...
    def qa_update(indices):
        ...
        for b in range(0, len(indices), micro):
            ...
            for j in group:
                ...
                ans, unl, fork = answer_span_ce_and_unl(train_rows[j])
                loss = loss + (mar + LAMBDA_PTR * ptr + LAMBDA_ANS * ans + LAMBDA_UNL * unl + LAMBDA_FORK * fork) / len(group)
    ...
    if args.mode == "smoke":
        ...
        payload = {
            ...
            "extra_forward_policy": "L_fork uses the existing teacher-forced answer logits; no generated-prefix rollout",
        }
    ...
    for update in range(resume_from + 1, TOTAL_UPDATES + 1):
        ...
        atomic_torch(ckdir / f"checkpoint_{update:04d}.pt", {
            "schema": "baby_vnext_phase2a_t29_disambiguation_fork_lr3p75e5_1k_model_v1",
            "update": update,
            "seed": args.seed,
            "phase": phase,
            "model_state_dict": model.state_dict(),
        ...
        if rec["hard_stop"] or rec["dev_ce"] > 1.50:
            final_class, stop_code = "T29_HARD_STOP", "integrity_or_catastrophe"
            break
        if rec["dev_ce"] > 1.30:
            final_class, stop_code = "T29_LANGUAGE_REGRESSION", "language_ce_gt_1_30"
            break
