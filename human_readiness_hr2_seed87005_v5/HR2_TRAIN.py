...
R=ROOT/'human_readiness_hr2_seed87005_v5'; DEVICE='cuda'
...
def main():
 ap=argparse.ArgumentParser(); ap.add_argument('--arm',choices=['factual','control'],required=True); ap.add_argument('--seed',type=int,default=87005); ap.add_argument('--output',required=True); ap.add_argument('--resume',action='store_true'); args=ap.parse_args(); assert args.seed==87005
 proto=json.loads((R/'HR2_PROTOCOL.json').read_text()); assert sha(Path(proto['parent_path']))==proto['parent_sha256']; assert torch.cuda.is_available()
 ...
 if args.resume:
  st=torch.load(out/'restart.pt',map_location=DEVICE,weights_only=False); assert st['arm']==args.arm and st['parent_sha256']==proto['parent_sha256'] and st['protocol_sha256']==sha(R/'HR2_PROTOCOL.json') and st['schedule_sha256']==sha(R/'ENGLISH_SCHEDULE.json'); m.load_state_dict(st['model_state']); opt.load_state_dict(st['optimizer_state']); random.setstate(st['python_rng']); torch.set_rng_state(st['torch_rng'].cpu()); start=int(st['completed_update'])+1; devs=st.get('dev_loss',devs)
