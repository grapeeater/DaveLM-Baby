"""Phase 2A T12: T9 fact-clause pointer, 9:1, lr 2.5e-5, 750 updates.
...
BUNDLE_DEFAULT = Path(r"C:\DaveLM-CADAVER\phase2a_t12_factclause_pointer_lr2p5e5_750_v1")
...
def main():
    ...
    if args.seed not in (710001, 710002, 710003):
    ...
    if scope != {"trainable": 40849664, "frozen": 20670721}:
    ...
    if args.resume:
        ...
        optimizer = torch.optim.AdamW(
            ...
            lr=2.5e-5, betas=(0.9, 0.999), eps=1e-8, weight_decay=0.05,
    ...
    for update in range(resume_from + 1, 751):
        ...
        atomic_torch(ckdir / f"checkpoint_{update:04d}.pt", {
            "schema": "baby_vnext_phase2a_t12_factclause_pointer_lr2p5e5_750_model_v1",
            "update": update,
            "seed": args.seed,
            "model_state_dict": model.state_dict(),
        })
        if rec["hard_stop"] or rec["dev_ce"] > 1.50:
            final_class, stop_code = "T12_HARD_STOP", "integrity_or_catastrophe"
            break
        if rec["dev_ce"] > 1.30:
            final_class, stop_code = "T12_LANGUAGE_REGRESSION", "language_ce_gt_1_30"
            break
        if update == 750:
