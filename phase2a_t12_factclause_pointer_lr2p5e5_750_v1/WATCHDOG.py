"""Detached T12 sequential watchdog. Never loads T3 TEST."""
...
RUNNER = Path(r"C:\DaveLM-CADAVER\phase2a_t12_factclause_pointer_lr2p5e5_750_v1\T12_RUNNER.py")
BUNDLE = Path(r"C:\DaveLM-CADAVER\phase2a_t12_factclause_pointer_lr2p5e5_750_v1")
ROOT = Path(r"C:\DaveLM-CADAVER")
SEEDS = (710001, 710002, 710003)
HEARTBEAT = BUNDLE / "WATCHDOG_HEARTBEAT.json"
LOG = ROOT / "phase2a_t12_factclause_pointer_lr2p5e5_750_v1_watchdog.log"
POLL = 20
...
def live_cmds():
    ...
    return [ln.strip() for ln in raw.splitlines()
            if "T12_RUNNER.py" in ln or ("t12_factclause" in ln.lower() and "WATCHDOG" in ln)]


def out_dir(seed):
    return ROOT / f"phase2a_t12_factclause_pointer_lr2p5e5_750_v1_run_seed{seed}"
...
def owner_live():
    return any("T12_RUNNER.py" in c for c in live_cmds())


def run_seed(seed, resume):
    out = out_dir(seed)
    log_path = ROOT / f"phase2a_t12_factclause_pointer_lr2p5e5_750_v1_train_seed{seed}.log"
    env = os.environ.copy()
...
def classify_study(results):
    ...
    if any(c == "T12_HARD_STOP" for c in classes):
        return "T12_HARD_STOP"
    if classes.count("T12_FULL_SUCCESS") >= 2:
        return "T12_FULL_SUCCESS"
    if sum(c in ("T12_FULL_SUCCESS", "T12_REPRESENTATION_SUCCESS_OUTPUT_FAIL") for c in classes) >= 2:
        return "T12_REPRESENTATION_SUCCESS_OUTPUT_FAIL"
    if classes.count("T12_LANGUAGE_REGRESSION") >= 2:
        return "T12_LANGUAGE_REGRESSION"
    if classes.count("T12_SHORTCUT_FAILURE") >= 2:
        return "T12_SHORTCUT_FAILURE"
    return "T12_FAIL_NO_REPRESENTATION"
...
def main():
    ...
    write_json(BUNDLE / "RUN_LEDGER.json", {
        ...
        "study": "BABY_VNEXT_PHASE2A_T12_FACTCLAUSE_POINTER_LR2P5E5_750_V1",
