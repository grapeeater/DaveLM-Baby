"""Phase 2A T31: T24 recipe plus an explicit ordered native source-copy bridge.
...
BUNDLE_DEFAULT = Path(r"C:\DaveLM-CADAVER\phase2a_t31_native_source_copy_bridge_v1")
PHASE_A_SCOPE = {"trainable": 40849668, "frozen": 20670721}
PHASE_B_SCOPE = {"trainable": 21163268, "frozen": 40357121}
REVERT_AFTER = 750
TOTAL_UPDATES = 1000
...
LAMBDA_FORK = 0.0
FORK_MARGIN = 1.0
AUTHORIZED_SEEDS = (880001, 880002, 880003)
CTX = 256
EOS = 3
...
def load_model():
    ...
    from T31_COPY import T31CopyModel
    ...
    if unexpected or missing != ["copy_gate"]:
        raise RuntimeError(f"T31 parent schema mismatch missing={missing} unexpected={unexpected}")
    with torch.no_grad():
        model.copy_gate.zero_()
...
def pointer_and_margin(model, row, device, train_mode: bool):
    ...
    for cand in row["candidate_token_ids"]:
        ...
        hh = model.base_model.forward_hidden(xx)
        _, ww, _ = model.pointer_route(hh, spans, p_len - 1)
        cp = model.copy_probs_for_row(row, ww, device, model.base_model.config.vocab_size)
        logits = model.logits_with_copy(hh, cp, p_len - 1)
...
def greedy_answer(model, row, device):
    ...
    for _ in range(len(target) + 2):
        ...
        _, weights, _ = model.pointer_route(hidden, spans, p_len - 1)
        cp = model.copy_probs_for_row(row, weights, device, model.base_model.config.vocab_size)
        logits = model.logits_with_copy(hidden, cp, p_len - 1)
        t = int(logits[0, -1].argmax())
        gen.append(t)
        out_ids.append(t)
        if t == EOS:
...
def futility_decision(rec: dict, update: int) -> tuple[bool, str]:
    ...
    active = float(rec.get("copy_gate_l2", 0.0)) > 0.01
...
def classify(rec: dict, u0_ce: float) -> str:
    ...
    if rep and max(sc.values()) >= (ptr / rec["dev"]["n"]) - 0.02:
        return "T31_SHORTCUT_FAILURE"
    nat = (rec["dev"]["native_forced_choice"] >= 96
           and rec["dev_reversals"]["native"] >= 48
           and rec["dev"]["exact_answer_eos"] >= 80)
    acq = rec["acq16_pass"]
    if not lang_ok:
        return "T31_LANGUAGE_REGRESSION"
    if not rep:
        return "T31_FAIL_NO_REPRESENTATION"
    if not nat or not acq:
        return "T31_REPRESENTATION_SUCCESS_OUTPUT_FAIL"
    return "T31_FULL_SUCCESS"
...
def main():
    ...
    model.to(device)
    binding_names = [n for n in model.state_dict() if not n.startswith("base_model.") and n != "copy_gate"]
    ...
    def run_full(update: int) -> dict:
        ...
        rec = {
            ...
            "scope": scope,
            "copy_gate_l2": float(torch.linalg.vector_norm(model.copy_gate.detach()).cpu()),
            "copy_gate_mean_abs": float(model.copy_gate.detach().abs().mean().cpu()),
            "test_loaded": False,
        }
    ...
    def answer_span_ce_and_unl(row):
        ...
        ids = prompt + target
        if len(ids) > CTX + 1:
            raise RuntimeError(f"T31 sequence exceeds frozen context for {row.get('id')}")
        x = torch.tensor([ids[:-1]], device=device)
        y = torch.tensor([ids[1:]], device=device)
        hidden = model.base_model.forward_hidden(x)
        spans = fact_clause_spans_bos(row)
        logits, _, _, _, _ = model.route_logits_for_row(hidden, row, spans, p_len, p_len - 1)
        n = len(target)
    ...
    if args.mode == "smoke":
        ...
        payload = {
            ...
            "extra_forward_policy": "persistent copy bridge reuses one base hidden; no generated-prefix rollout",
            "copy_parameters": int(model.copy_gate.numel()),
        }
    ...
    for update in range(resume_from + 1, TOTAL_UPDATES + 1):
        ...
        bad = [n for n, p in model.named_parameters()
               if p.grad is not None and (freeze_early(n, phase) or (not n.startswith("base_model.") and n != "copy_gate"))]
        if bad:
        ...
        atomic_torch(ckdir / f"checkpoint_{update:04d}.pt", {
            "schema": "baby_vnext_phase2a_t31_native_source_copy_bridge_lr3p75e5_1k_model_v1",
            "update": update,
            "seed": args.seed,
            "phase": phase,
            "model_state_dict": model.state_dict(),
        ...
        if rec["hard_stop"] or rec["dev_ce"] > 1.50:
            final_class, stop_code = "T31_HARD_STOP", "integrity_or_catastrophe"
            break
        if fut_stop:
            final_class, stop_code = "T31_FUTILITY_STOP", "native_trajectory_futility_u750"
            break
        if rec["dev_ce"] > 1.30:
            final_class, stop_code = "T31_LANGUAGE_REGRESSION", "language_ce_gt_1_30"
            break
