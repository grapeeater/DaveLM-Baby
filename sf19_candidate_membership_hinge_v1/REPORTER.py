"""Frozen SF19 reporter with explicit U0/U100/U200 rows."""
import json
...
def classify(rows):
 ...
 if sum(r['gates']['endpoint_pass'] for r in rows)>=2: return 'CANDIDATE_MEMBERSHIP_OBJECTIVE_SUPPORTED'
 if sum((not r['gates']['d3']) and r['widening_movement'] for r in rows)>=2: return 'CANDIDATE_MEMBERSHIP_OBJECTIVE_INSUFFICIENT'
 if sum(r['completed']==200 and r['gates']['d3'] and retention(r['gates']) and not r['gates']['dev_surface'] and not r['gates']['dev_order'] for r in rows)>=2: return 'WIDENING_STALLED_UNDER_MEMBERSHIP'
...
def main():
 ...
 lines=['# SF19 FINAL CLASSIFICATION','',f'**{cls}**','','# SOLE SCIENTIFIC CHANGE','','Relative to sealed SF14, SF19 added a bounded relative hinge requiring the better of the two valid answer candidates to outrank every noncandidate vocabulary token by 1 nat. First-answer CE remained zero; later CE, pairwise margin, broad KL, data, optimizer, scope, schedule, gates, binding, and evaluation remained unchanged.','', '# PER-SEED TRAJECTORY','', '| Seed | U | TRAIN16 c/e/r/f | Surface c/e | Order c/e (fact/copy) | Language CE/PPL | D3 | Binding |','|---:|---:|---:|---:|---:|---:|---:|---|']
 for r in rows:
 ...
 if cls=='CANDIDATE_MEMBERSHIP_OBJECTIVE_SUPPORTED': lines.append('The bounded candidate-membership objective was sufficient in at least two branches to satisfy the complete frozen endpoint. This claim is confined to SF19.')
 elif cls=='CANDIDATE_MEMBERSHIP_OBJECTIVE_INSUFFICIENT': lines.append('The bounded candidate-membership objective did not contain replicated D3 leakage while retention and widening remained active in at least two branches.')
 elif cls=='WIDENING_STALLED_UNDER_MEMBERSHIP': lines.append('The objective contained D3 and retention but widening stalled below both frozen endpoint gates in at least two branches.')
