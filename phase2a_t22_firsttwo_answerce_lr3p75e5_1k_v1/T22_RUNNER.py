"""Phase 2A T22: T18 two-phase recipe plus first-two-token answer-span CE.

First TWO answer tokens CE through the base. Remaining name/period/EOS CE
stop-grads through language_head only. Uniform mean lambda_ans=1.0.
...
BUNDLE_DEFAULT = Path(r"C:\DaveLM-CADAVER\phase2a_t22_firsttwo_answerce_lr3p75e5_1k_v1")
...
def main():
    ...
    if args.seed not in (810001, 810002, 810003):
    ...
    def answer_span_ce(row):
        ...
        n = len(target)
        k = min(2, n)
        ce_head = F.cross_entropy(
            logits_full[0, -n:-n + k, :], y[0, -n:-n + k], reduction="sum")
        if n == k:
            return ce_head / n
        ce_tail = F.cross_entropy(
            logits_readout[0, -n + k:, :], y[0, -n + k:], reduction="sum")
        return (ce_head + ce_tail) / n
    ...
    for update in range(resume_from + 1, TOTAL_UPDATES + 1):
        ...
        if rec["hard_stop"] or rec["dev_ce"] > 1.50:
            final_class, stop_code = "T22_HARD_STOP", "integrity_or_catastrophe"
            break
        if rec["dev_ce"] > 1.30:
            final_class, stop_code = "T22_LANGUAGE_REGRESSION", "language_ce_gt_1_30"
            break
