"""Phase 2A T7: T6 late-base pointer with paired-assignment QA batches.
...
BUNDLE_DEFAULT = Path(r"C:\DaveLM-CADAVER\phase2a_t7_paired_reversal_pointer_v1")
...
def main():
    ...
    if args.seed not in (660001, 660002, 660003):
    ...
    for update in range(resume_from + 1, 501):
        ...
        atomic_torch(ckdir / f"checkpoint_{update:04d}.pt", {
            "schema": "baby_vnext_phase2a_t7_paired_reversal_pointer_model_v1",
            "update": update,
            "seed": args.seed,
            "model_state_dict": model.state_dict(),
        })
        if rec["hard_stop"] or rec["dev_ce"] > 1.50:
            final_class, stop_code = "T7_HARD_STOP", "integrity_or_catastrophe"
            break
        if update == 500:
