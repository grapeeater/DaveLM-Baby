"""Detached T7 sequential watchdog. Never loads T3 TEST."""
...
RUNNER = Path(r"C:\DaveLM-CADAVER\phase2a_t7_paired_reversal_pointer_v1\T7_RUNNER.py")
BUNDLE = Path(r"C:\DaveLM-CADAVER\phase2a_t7_paired_reversal_pointer_v1")
ROOT = Path(r"C:\DaveLM-CADAVER")
SEEDS = (660001, 660002, 660003)
HEARTBEAT = BUNDLE / "WATCHDOG_HEARTBEAT.json"
LOG = ROOT / "phase2a_t7_paired_reversal_pointer_v1_watchdog.log"
POLL = 20
...
def live_cmds():
    ...
    return [ln.strip() for ln in raw.splitlines()
            if "T7_RUNNER.py" in ln or ("t7_paired" in ln.lower() and "WATCHDOG" in ln)]


def out_dir(seed):
    return ROOT / f"phase2a_t7_paired_reversal_pointer_v1_run_seed{seed}"
...
def owner_live():
    return any("T7_RUNNER.py" in c for c in live_cmds())


def run_seed(seed, resume):
    out = out_dir(seed)
    log_path = ROOT / f"phase2a_t7_paired_reversal_pointer_v1_train_seed{seed}.log"
    env = os.environ.copy()
...
def classify_study(results):
    ...
    if any(c == "T7_HARD_STOP" for c in classes):
        return "T7_HARD_STOP"
    if classes.count("T7_FULL_SUCCESS") >= 2:
        return "T7_FULL_SUCCESS"
    if sum(c in ("T7_FULL_SUCCESS", "T7_REPRESENTATION_SUCCESS_OUTPUT_FAIL") for c in classes) >= 2:
        return "T7_REPRESENTATION_SUCCESS_OUTPUT_FAIL"
    if classes.count("T7_LANGUAGE_REGRESSION") >= 2:
        return "T7_LANGUAGE_REGRESSION"
    if classes.count("T7_SHORTCUT_FAILURE") >= 2:
        return "T7_SHORTCUT_FAILURE"
    return "T7_FAIL_NO_REPRESENTATION"
...
def main():
    ...
    write_json(BUNDLE / "RUN_LEDGER.json", {
        ...
        "study": "BABY_VNEXT_PHASE2A_T7_PAIRED_REVERSAL_POINTER_V1",
        ...
        "qa_batch_structure": "16 assignment pairs (32 items); pair members adjacent",
    })
    while True:
