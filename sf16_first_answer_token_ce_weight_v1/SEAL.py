...
def sha(p):
 ...
 return h.hexdigest()
def main():
 assert json.loads((H/'PREFLIGHT.json').read_text())['status']=='SF16_PROSPECTIVE_PREFLIGHT_PASS'
 ...
 rec={'status':'SF16_PROSPECTIVE_PREFLIGHT_PASS','study':'SF16_FIRST_ANSWER_TOKEN_CE_INTERPOLATION_V1','manifest_sha256':mh,'sealed_utc':datetime.now(timezone.utc).isoformat(),'sole_scientific_change':'first answer-name CE weight changed from 0.25 to 0.125 on English updates','checkpoint_loaded':False,'optimizer_created':False,'updates':0,'final_accessed':False,'sacred_accessed':False}
