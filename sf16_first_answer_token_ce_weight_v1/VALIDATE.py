"""Minimal mechanical SF16 preflight. No checkpoint load, optimizer, inference, or update."""
import ast
import hashlib
import json
import platform
...
SF15 = H.parent / "sf15_partial_first_token_ce_v1"
...
def main():
    ...
    old = json.loads((SF15 / "PROTOCOL.json").read_text(encoding="utf-8"))
    unchanged = [
    ...
    assert all(sha(H / n) == sha(SF15 / n) for n in unchanged)
    checks["byte_identical_sf15_scientific_payload"] = unchanged
    ...
    for row, rid in enumerate(batch["ids"]):
        ...
        assert float(weights[row, pos]) == 0.125
    ...
    loss_0125, meta = C.weighted_english_ce(logits, labels, batch["ids"], index)
    ...
    assert meta == {"first_answer_positions": 36, "later_response_eos_positions": 144, "ce_weight_sum": 148.5}
    ...
    checks["isolated_variable"] = {
        "first_answer_positions_weight_0_125": 36,
    ...
    inherited = json.loads((SF15 / "U0_PREFLIGHT_RECORD.json").read_text(encoding="utf-8"))
    mapping = {"87041": "87038", "87042": "87039", "87043": "87040"}
    branches = {}
    for seed, prior in mapping.items():
        record = dict(inherited["branches"][prior])
        record["sf16_seed"] = int(seed)
        record["source_sf15_preflight_seed"] = int(prior)
        branches[seed] = record
    u0 = {
        "study": "SF16_FIRST_ANSWER_TOKEN_CE_INTERPOLATION_V1",
        "record_type": "INHERITED_ACTUAL_U0_PARENT_REPRODUCTION",
        "source_path": str(SF15 / "U0_PREFLIGHT_RECORD.json"),
        "source_sha256": sha(SF15 / "U0_PREFLIGHT_RECORD.json"),
        "basis": "Same parent bytes and byte-identical evaluator/data; every sealed SF16 run independently reruns and asserts U0 before optimizer construction.",
        "branches": branches,
    }
    ...
    result = {
        "status": "SF16_PROSPECTIVE_PREFLIGHT_PASS",
        "checks": checks,
        "warnings": [
            "The 0.125 CE weight is normalized by total token weight. This exact interpolation reproduces SF14 at weight 0 and SF13 at weight 1.",
    ...
    (H / "PREFLIGHT.md").write_text(
        "# SF16 prospective preflight\n\n"
        "**SF16_PROSPECTIVE_PREFLIGHT_PASS**\n\n"
        "The sole scientific change from sealed SF15 is first-answer-name CE weight `0.25 -> 0.125`. "
        ...
        "A frozen 36-record English batch contained exactly 36 first-name positions at weight 0.125 and 144 later response/punctuation/EOS positions at weight 1.0. "
