...
def verify(sealed=True):
    if sealed:
        ...
        assert receipt['status'] == 'SF16_PROSPECTIVE_PREFLIGHT_PASS'
...
def main():
    ...
    if a.mode == 'preload':
        print('SF16_PRE_PARENT_LOAD_PASS', a.seed, flush=True); return
    ...
    for u in schedule[completed:]:
        ...
        if u['kind'] == 'english':
            ...
            assert abs(ce_meta['ce_weight_sum'] - len(u['ids']) * 4.125) < 1e-5
