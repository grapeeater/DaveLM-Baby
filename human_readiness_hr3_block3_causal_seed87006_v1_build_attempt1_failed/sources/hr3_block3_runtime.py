...
def flatten_quartets(data: dict[str, Any]) -> dict[str, dict[str, Any]]:
    return {quartet["quartet_id"]: quartet for quartet in data["quartets"]}


def binding_docs_for_batch(quartets: dict[str, dict[str, Any]], quartet_ids: list[str]) -> list[dict[str, Any]]:
    docs: list[dict[str, Any]] = []
    for quartet_id in quartet_ids:
        assert quartet_id in quartets, f"unknown frozen quartet identity: {quartet_id}"
        docs.extend(quartets[quartet_id]["docs"])
    return docs
