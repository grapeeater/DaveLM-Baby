...
def main() -> None:
    ...
    known_quartets = {q["quartet_id"] for q in rehearsal}
    assert len(binding_schedule) == 50 and all(len(batch["quartet_ids"]) == 8 for batch in binding_schedule)
    assert all(qid in known_quartets for batch in binding_schedule for qid in batch["quartet_ids"])
    ...
    assert "aligned_tensors" in trainer and "aligned_tensors" in evaluator
    ...
    assert "checkpoint_500" in trainer and "checkpoint_500" in evaluator
