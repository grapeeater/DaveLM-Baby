"""Phase 2A T11: T9 fact-clause pointer, 4:1 language, 750 updates.
...
BUNDLE_DEFAULT = Path(r"C:\DaveLM-CADAVER\phase2a_t11_factclause_pointer_4to1_750_v1")
...
def main():
    ...
    if args.seed not in (700001, 700002, 700003):
    ...
    if len(kinds) != 750 or kinds.count("lang") != 150 or kinds.count("qa") != 600:
        raise RuntimeError(f"kinds freeze mismatch len={len(kinds)}")
    if len(qa_sched) != 600 or any(len(b) != 32 for b in qa_sched):
        raise RuntimeError(f"qa schedule freeze mismatch len={len(qa_sched)}")
    if len(rehear) != 150 or any(len(x) != 64 for x in rehear):
    ...
    for update in range(resume_from + 1, 751):
        ...
        atomic_torch(ckdir / f"checkpoint_{update:04d}.pt", {
            "schema": "baby_vnext_phase2a_t11_factclause_pointer_4to1_750_model_v1",
            "update": update,
            "seed": args.seed,
            "model_state_dict": model.state_dict(),
        })
        if rec["hard_stop"] or rec["dev_ce"] > 1.50:
            final_class, stop_code = "T11_HARD_STOP", "integrity_or_catastrophe"
            break
        if rec["dev_ce"] > 1.30:
            final_class, stop_code = "T11_LANGUAGE_REGRESSION", "language_ce_gt_1_30"
            break
        if update == 750:
