"""Detached T11 sequential watchdog. Never loads T3 TEST."""
...
RUNNER = Path(r"C:\DaveLM-CADAVER\phase2a_t11_factclause_pointer_4to1_750_v1\T11_RUNNER.py")
BUNDLE = Path(r"C:\DaveLM-CADAVER\phase2a_t11_factclause_pointer_4to1_750_v1")
ROOT = Path(r"C:\DaveLM-CADAVER")
SEEDS = (700001, 700002, 700003)
HEARTBEAT = BUNDLE / "WATCHDOG_HEARTBEAT.json"
LOG = ROOT / "phase2a_t11_factclause_pointer_4to1_750_v1_watchdog.log"
POLL = 20
...
def live_cmds():
    ...
    return [ln.strip() for ln in raw.splitlines()
            if "T11_RUNNER.py" in ln or ("t11_factclause" in ln.lower() and "WATCHDOG" in ln)]


def out_dir(seed):
    return ROOT / f"phase2a_t11_factclause_pointer_4to1_750_v1_run_seed{seed}"
...
def owner_live():
    return any("T11_RUNNER.py" in c for c in live_cmds())


def run_seed(seed, resume):
    out = out_dir(seed)
    log_path = ROOT / f"phase2a_t11_factclause_pointer_4to1_750_v1_train_seed{seed}.log"
    env = os.environ.copy()
...
def classify_study(results):
    ...
    if any(c == "T11_HARD_STOP" for c in classes):
        return "T11_HARD_STOP"
    if classes.count("T11_FULL_SUCCESS") >= 2:
        return "T11_FULL_SUCCESS"
    if sum(c in ("T11_FULL_SUCCESS", "T11_REPRESENTATION_SUCCESS_OUTPUT_FAIL") for c in classes) >= 2:
        return "T11_REPRESENTATION_SUCCESS_OUTPUT_FAIL"
    if classes.count("T11_LANGUAGE_REGRESSION") >= 2:
        return "T11_LANGUAGE_REGRESSION"
    if classes.count("T11_SHORTCUT_FAILURE") >= 2:
        return "T11_SHORTCUT_FAILURE"
    return "T11_FAIL_NO_REPRESENTATION"
...
def main():
    ...
    write_json(BUNDLE / "RUN_LEDGER.json", {
        ...
        "study": "BABY_VNEXT_PHASE2A_T11_FACTCLAUSE_POINTER_4TO1_750_V1",
