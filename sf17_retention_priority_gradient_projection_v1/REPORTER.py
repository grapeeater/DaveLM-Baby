"""Frozen SF17 reporter with explicit U0/U100/U200 rows."""
import json
...
def classify(rows):
 ...
 if sum(r['gates']['endpoint_pass'] for r in rows)>=2: return 'RETENTION_PRIORITY_PROJECTION_SUPPORTED'
 if sum((not r['gates']['d3']) and r['widening_movement'] for r in rows)>=2: return 'RETENTION_PRIORITY_PROJECTION_INSUFFICIENT'
 if sum(r['completed']==200 and r['gates']['d3'] and retention(r['gates']) and not r['gates']['dev_surface'] and not r['gates']['dev_order'] for r in rows)>=2: return 'WIDENING_STALLED_UNDER_PROJECTION'
 return 'MIXED_OR_UNRESOLVED'
def main():
 p=C.verify(); rows=[]
 for seed in p['seeds']:
  d=H/'runs'/f'seed_{seed}_curriculum'; st=C.read(d/'STATUS.json'); us=[0,100]+([200] if st['completed']==200 else []); ss=[snap(d,u) for u in us]; metrics=[json.loads(x) for x in (d/'TRAIN_METRICS.jsonl').read_text().splitlines()]; em=[x for x in metrics if x['kind']=='english']; assert len(metrics)==st['completed']
  tele={'english_updates':len(em),'conflict_updates':sum(x['gradient_conflict'] for x in em),'conflict_fraction':sum(x['gradient_conflict'] for x in em)/len(em),'mean_gradient_cosine':sum(x['gradient_cosine'] for x in em)/len(em),'mean_projection_coefficient':sum(x['projection_coefficient'] for x in em)/len(em)}
  rows.append({'seed':seed,'status':st['status'],'completed':st['completed'],'checkpoint':st['checkpoint'],'checkpoint_sha256':st['checkpoint_sha256'],'gates':st['gates'],'snapshots':ss,'terminal':ss[-1],'projection':tele,'widening_movement':ss[-1]['surface']['exact']>ss[0]['surface']['exact'] and ss[-1]['order']['exact']>ss[0]['order']['exact']})
 ...
 lines=['# SF17 FINAL CLASSIFICATION','',f'**{cls}**','','# SOLE SCIENTIFIC CHANGE','','Relative to sealed SF13 full-CE training, SF17 preserved the KL gradient and projected away only the globally opposing component of factual CE+margin gradients on English updates. All loss values/weights, data, optimizer, scope, schedule, gates, binding, and evaluation remained unchanged.','', '# PER-SEED TRAJECTORY','', '| Seed | U | TRAIN16 c/e/r/f | Surface c/e | Order c/e (fact/copy) | Language CE/PPL | D3 | Binding |','|---:|---:|---:|---:|---:|---:|---:|---|']
 for r in rows:
 ...
 lines+=['','# PROJECTION TELEMETRY','']
 for r in rows:
  q=r['projection']; lines.append(f"- {r['seed']}: conflicts {q['conflict_updates']}/{q['english_updates']} ({q['conflict_fraction']:.1%}); mean cosine `{q['mean_gradient_cosine']:.5f}`; mean coefficient `{q['mean_projection_coefficient']:.5g}`.")
 ...
 if cls=='RETENTION_PRIORITY_PROJECTION_SUPPORTED': lines.append('The tested retention-priority projection was sufficient in at least two branches to satisfy the complete frozen endpoint. This claim is confined to the SF17 recipe and panels.')
 elif cls=='RETENTION_PRIORITY_PROJECTION_INSUFFICIENT': lines.append('The tested retention-priority projection did not contain replicated D3 leakage while retention and widening remained active in at least two branches.')
 elif cls=='WIDENING_STALLED_UNDER_PROJECTION': lines.append('The projection contained D3 and retention but widening stalled below both frozen endpoint gates in at least two branches.')
