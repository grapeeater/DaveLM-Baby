...
def sha(p):
 ...
 return h.hexdigest()
def main():
 assert json.loads((H/'PREFLIGHT.json').read_text())['status']=='SF17_PROSPECTIVE_PREFLIGHT_PASS'
 ...
 rec={'status':'SF17_PROSPECTIVE_PREFLIGHT_PASS','study':'SF17_RETENTION_PRIORITY_GRADIENT_PROJECTION_V1','manifest_sha256':mh,'sealed_utc':datetime.now(timezone.utc).isoformat(),'sole_scientific_change':'retention-priority global conflict projection on English gradients','checkpoint_loaded':False,'optimizer_created':False,'updates':0,'final_accessed':False,'sacred_accessed':False}
