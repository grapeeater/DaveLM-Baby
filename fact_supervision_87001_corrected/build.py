...
class Corpus:
    ...
    def any_overlap(self, rows):
        for r in rows:
            if norm(r['prompt']) in self.joined:
                return True
            for c in r.get('candidates', []):
                if norm(r['prompt'] + c) in self.joined:
                    return True
        return False
        ...
        for stratum in ('object', 'predicate'):
            ...
            for f in subset:
                for a, q, o, position in itertools.product((0, 1), (0, 1), (0, 1), ('before', 'after')):
                    ...
                    independent.check_row(r)
                    controlled.append(r)
        # Naturalistic diagnostic selection is outcome-blind and independent per holdout:
        # choose the first eight object families in deterministic family-id order whose
        # two assignment/order-zero prompts are unique within this holdout and whose
        # prompts do not intersect the other holdout. This is the sole authorized
        # correction to the failed construction.
        natural_candidates = sorted((f for f in fs if f['stratum'] == 'object'), key=lambda f: canonical(family_identity(f)))
        selected = []
        seen = set()
        for f in natural_candidates:
            candidate_prompts = [f'{n} {v} the {d} and then went home.' for n, v, d in facts_for(f, 0)]
            if any(p in seen for p in candidate_prompts):
                continue
            selected.append((f, candidate_prompts))
            seen.update(candidate_prompts)
            if len(selected) == 8:
                break
        assert len(selected) == 8, f'Could not select eight unique naturalistic families for {partition}'
        for f, _ in selected:
...
def main():
    ...
    transfers, natural = transfer_rows(families, tok)
    ...
    out = HERE / 'construction_check_v2.json'
    ...
    payload = HERE / 'construction_candidates_v2.json'
