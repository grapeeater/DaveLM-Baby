"""Frozen SF14 aggregation and prospective classification."""
import hashlib,json
...
def classify(rows):
 ...
 if sum(r['gates']['endpoint_pass'] for r in rows)>=2: return 'FIRST_TOKEN_CE_ABLATION_SUPPORTED'
 insufficient=sum((not r['gates']['d3']) and r['widening_movement'] for r in rows)
 if insufficient>=2: return 'FIRST_TOKEN_CE_ABLATION_INSUFFICIENT'
...
def main():
 ...
 if cls=='FIRST_TOKEN_CE_ABLATION_SUPPORTED': lines.append('Removing first-answer-name causal CE was sufficient under SF14 to preserve D3 while retaining acquisition, language, binding, and endpoint widening. This does not prove CE is fundamentally incompatible or that margin is necessary and sufficient.')
 elif cls=='FIRST_TOKEN_CE_ABLATION_INSUFFICIENT': lines.append('Direct first-answer-token causal CE pressure was insufficient to explain or contain the replicated D3 regression under SF14. The ablation preserved other tested capabilities and widening moved, but D3 still failed in at least two branches.')
 elif cls=='RETENTION_REGRESSION': lines.append('SF14 failed a frozen TRAIN16, language, or binding retention requirement; safety failure takes precedence.')
 else: lines.append('The valid SF14 branches did not produce a decisive preregistered pattern.')
