...
def validate(items):
    priors=[r for r in items if r['section']=='query_only_prior']
    assert len(priors)==16 and Counter(r['prior_reference_index'] for r in priors)=={0:8,1:8}
    for r in priors:
        assert '\n' not in r['prompt'] and r['prompt'].startswith('Who ') and r['prompt'].endswith('?')
        assert len(r['candidates'])==2 and r['candidates']==[' Sam.',' Tom.']
    ctrl=[r for r in items if r['section'] in ('near_distribution','counterfactual','surface_form','distractor')]; groups={}
    for r in ctrl:
