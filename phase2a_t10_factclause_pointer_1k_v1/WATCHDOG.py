"""Detached T10 sequential watchdog. Never loads T3 TEST."""
...
RUNNER = Path(r"C:\DaveLM-CADAVER\phase2a_t10_factclause_pointer_1k_v1\T10_RUNNER.py")
BUNDLE = Path(r"C:\DaveLM-CADAVER\phase2a_t10_factclause_pointer_1k_v1")
ROOT = Path(r"C:\DaveLM-CADAVER")
SEEDS = (690001, 690002, 690003)
HEARTBEAT = BUNDLE / "WATCHDOG_HEARTBEAT.json"
LOG = ROOT / "phase2a_t10_factclause_pointer_1k_v1_watchdog.log"
POLL = 20
...
def live_cmds():
    ...
    return [ln.strip() for ln in raw.splitlines()
            if "T10_RUNNER.py" in ln or ("t10_factclause" in ln.lower() and "WATCHDOG" in ln)]


def out_dir(seed):
    return ROOT / f"phase2a_t10_factclause_pointer_1k_v1_run_seed{seed}"
...
def owner_live():
    return any("T10_RUNNER.py" in c for c in live_cmds())


def run_seed(seed, resume):
    out = out_dir(seed)
    log_path = ROOT / f"phase2a_t10_factclause_pointer_1k_v1_train_seed{seed}.log"
    env = os.environ.copy()
...
def classify_study(results):
    ...
    if any(c == "T10_HARD_STOP" for c in classes):
        return "T10_HARD_STOP"
    if classes.count("T10_FULL_SUCCESS") >= 2:
        return "T10_FULL_SUCCESS"
    if sum(c in ("T10_FULL_SUCCESS", "T10_REPRESENTATION_SUCCESS_OUTPUT_FAIL") for c in classes) >= 2:
        return "T10_REPRESENTATION_SUCCESS_OUTPUT_FAIL"
    if classes.count("T10_LANGUAGE_REGRESSION") >= 2:
        return "T10_LANGUAGE_REGRESSION"
    if classes.count("T10_SHORTCUT_FAILURE") >= 2:
        return "T10_SHORTCUT_FAILURE"
    return "T10_FAIL_NO_REPRESENTATION"
...
def main():
    ...
    write_json(BUNDLE / "RUN_LEDGER.json", {
        ...
        "study": "BABY_VNEXT_PHASE2A_T10_FACTCLAUSE_POINTER_1K_V1",
