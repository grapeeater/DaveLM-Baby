"""Phase 2A T10: T9 fact-clause pointer, 1000 updates.
...
BUNDLE_DEFAULT = Path(r"C:\DaveLM-CADAVER\phase2a_t10_factclause_pointer_1k_v1")
...
def main():
    ...
    if args.seed not in (690001, 690002, 690003):
    ...
    for update in range(resume_from + 1, 1001):
        ...
        atomic_torch(ckdir / f"checkpoint_{update:04d}.pt", {
            "schema": "baby_vnext_phase2a_t10_factclause_pointer_1k_model_v1",
            "update": update,
            "seed": args.seed,
            "model_state_dict": model.state_dict(),
        })
        if rec["hard_stop"] or rec["dev_ce"] > 1.50:
            final_class, stop_code = "T10_HARD_STOP", "integrity_or_catastrophe"
            break
        if rec["dev_ce"] > 1.30:
            final_class, stop_code = "T10_LANGUAGE_REGRESSION", "language_ce_gt_1_30"
            break
        if update == 1000:
