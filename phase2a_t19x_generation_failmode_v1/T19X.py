"""Read-only T19 generation fail-mode vs T18X.

Does not train. Does not load T3 TEST.
"""
...
OUT = ROOT / "phase2a_t19x_generation_failmode_v1"
T19 = ROOT / "phase2a_t19_suffix_answerce_lr3p75e5_1k_v1"
T3_DATA = ROOT / "phase2a_t3_rebuilt_study_v1" / "data"
sys.path.insert(0, str(T19))
sys.path.insert(0, str(ROOT / "baby_vnext_60m_design_v1"))

from T19_RUNNER import ARCH, CTX, EOS, TOK_PATH, TOK_SHA, load_jsonl, sha256  # noqa: E402
...
def main():
    ...
    for seed in SEEDS:
        run = ROOT / f"phase2a_t19_suffix_answerce_lr3p75e5_1k_v1_run_seed{seed}"
