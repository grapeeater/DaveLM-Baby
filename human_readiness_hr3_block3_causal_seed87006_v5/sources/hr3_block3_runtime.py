...
def atomic_torch_save(value: Any, path: Path) -> None:
    ...
    try:
        ...
        # Windows rejects fsync on the read-only descriptor used in the first
        # mechanical implementation.  Open read/write so the durable commit
        # operation has exactly the intended semantics on the pinned runtime.
...
def restore_rng(state: dict[str, Any]) -> None:
    ...
    # Restart loads use map_location=cuda; the CPU generator state must be
    # moved back to a CPU byte tensor before torch.set_rng_state.
