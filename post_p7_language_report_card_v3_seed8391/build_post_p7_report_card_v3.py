...
OUT = ROOT / 'post_p7_language_report_card_v3_seed8391'
SEED = 8391
...
class FallbackTokenizer:
    ...
    def encode(self,s):
        ...
        chunks=re.findall(r"\s+|[A-Za-z]+|[0-9]+|[^A-Za-z0-9\s]",s)
        toks=[]
        for c in chunks:
...
def main():
    ...
    assert len(items)==272 and len({norm(r['prompt']) for r in items})==272
    val=validate(items)
    for r in items:
    ...
    for p in sources:
        ...
        audits.append({'path':str(p),'sha256':sha(p),'prompt_overlap':po,'completed_overlap':co}); overlaps += [x for x in po+co if not x.startswith('naturalistic:')]
    assert not overlaps, overlaps
    OUT.mkdir(); dump('PROTOCOL.md',PROTOCOL); dump('FAMILIES.json',fams); dump('ITEMS.jsonl',''.join(json.dumps(r,ensure_ascii=False,separators=(',',':'))+'\n' for r in items)); dump('LEXICON.json',{'subjects':names,'objects':objs,'colors':colors,'relations':['found','carried'],'candidate_token_ids':{' Sam.':tok.encode(' Sam.').ids,' Tom.':tok.encode(' Tom.').ids}}); dump('OVERLAP_AUDIT.json',{'normalization':'casefold and collapse whitespace','sources':audits,'prompt_overlaps':0,'completed_overlaps':0,'limitation':'exact non-overlap does not exclude semantic or paraphrase contamination'}); dump('PREFLIGHT.json',{'status':'PASS_NO_MODEL_ACCESS','validator':val,'items':len(items),'controlled_items':256,'primary_matched_items':192,'distractor_items':64,'naturalistic_items':16,'unique_prompts':len(items),'equal_candidate_tokens':3,'tokenizer_sha256':sha(TOK_PATH),'sacred_accessed':False,'model_accessed':False,'training':False})
    dump('MANIFEST.json',{'version':3,'seed':SEED,'counts':{'near_distribution':96,'counterfactual':48,'surface_form':48,'distractor':64,'naturalistic':16,'controlled':256,'total':272},'tokenizer':{'path':str(TOK_PATH),'sha256':sha(TOK_PATH),'byte_fallback':False},'parent_checkpoints_behavior_accessed':False})
