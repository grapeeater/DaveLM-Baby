...
ROOT=Path(r'C:\DaveLM-CADAVER'); PILOT=ROOT/'language_pilot_0_tinystories_seed8380'; GRAD=ROOT/'treatment13_orthogonal_shared_unbounded_seed8380'; CK=GRAD/'checkpoints/orthogonal_shared_unbounded/seed_8380/latest.pt'; TOK=Path(r'C:\DaveLM-v0.9\tokenizer\v0_7\davelm_tokenizer.json'); SRC=ROOT/'treatment13_learned_mapping_row_localization_seed8380'; OUT=PILOT/'pilot_run'; sys.path.insert(0,str(ROOT)); sys.path.insert(0,r'C:\DaveLM-v0.9')
...
def main():
 OUT.mkdir(parents=True,exist_ok=True); tok=Tokenizer.from_file(str(TOK)); assert sha(CK)=='fed298748e62def6f1751ef1bb7df0cdec51d53fac6e4c86e8c00a95dccdd430' and sha(TOK)=='e1c18bae74f6d502c0012953b3eef63f787cefd41c9a47b94e803e665dab343b'; man=json.loads((PILOT/'CORPUS_MANIFEST.json').read_text()); tr=[json.loads(x) for x in (PILOT/'language_train.jsonl').read_text(encoding='utf-8').splitlines()]; dv=[json.loads(x) for x in (PILOT/'language_dev.jsonl').read_text(encoding='utf-8').splitlines()]; rp=json.loads((PILOT/'binding_rehearsal.json').read_text())['quartets']; dp=json.loads((PILOT/'binding_dev.json').read_text())['quartets']; rehearsal=[d for q in rp for d in q['docs']]; binddev=[d for q in dp for d in q['docs']];
 dev=torch.device('cuda'); model=load_model(dev); model.eval(); stream_tr=torch.tensor(encode_documents(tok,[{'text':x['text']} for x in tr]),dtype=torch.long); stream_dv=torch.tensor(encode_documents(tok,[{'text':x['text']} for x in dv]),dtype=torch.long); prompts=['Hello','Hello, my name is','What is your name?','The dog is','The girl went to','Yesterday I','I am happy because','Why did the dog run?','Tell me a short story about a dog.','he womputeld']; pre={'language_loss':lang_eval(model,stream_dv,dev,SEED+1001),'language_perplexity':math.exp(lang_eval(model,stream_dv,dev,SEED+1001)),'raw_generations':[generate(model,tok,p) for p in prompts],'binding':binding_eval(model,binddev,dev)}; (OUT/'BASELINE_RESULTS.json').write_text(json.dumps(pre,indent=2),encoding='utf-8')
 spec={'pilot':'Language Pilot 0','seed':SEED,'starting_checkpoint_sha256':sha(CK),'tokenizer_sha256':sha(TOK),'language_train_sha256':sha(PILOT/'language_train.jsonl'),'language_dev_sha256':sha(PILOT/'language_dev.jsonl'),'binding_rehearsal_sha256':sha(PILOT/'binding_rehearsal.json'),'binding_dev_sha256':sha(PILOT/'binding_dev.json'),'optimizer':'AdamW','learning_rate':3e-4,'weight_decay':.05,'gradient_clip':2.0,'language_batch_size':64,'language_context':256,'binding_batch_size':32,'total_updates':1000,'language_updates':900,'binding_updates':100,'schedule':'9 language updates then 1 binding update','lr_schedule':'constant 3e-4 (T13 precedent; no mixed warmup adaptation)','objective_binding':'answer CE + existing hard-min permutation-invariant localization loss','language_scope':'base_model only; T13 localizer/retrieval frozen','binding_scope':'full model','language_sampling':'torch Generator seed 8380 random windows','binding_sampling':'precomputed deterministic 100-step schedule, 8 quartets per step, seed 8380'}; spectext=json.dumps(spec,sort_keys=True,indent=2); (OUT/'TRAINING_SPEC.json').write_text(spectext,encoding='utf-8'); spech=hashlib.sha256(spectext.encode()).hexdigest()
 torch.manual_seed(SEED); random.seed(SEED); opt=torch.optim.AdamW(model.parameters(),lr=3e-4,weight_decay=.05); model.train(); bind_order=list(range(len(rp))); rng=random.Random(8380); bsched=[]
 for ep in range(10):
  rng.shuffle(bind_order)
  for j in range(0,len(bind_order),8): bsched.append([bind_order[k] for k in bind_order[j:j+8]])
 bq={q['quartet_id']:q for q in rp}; lg=torch.Generator().manual_seed(SEED); metrics=[]; bi=0; t=time.time()
 for step in range(1,1001):
  is_bind=step%10==0
  if is_bind:
   docs=[]
   for qi in bsched[bi]: docs.extend(bq[f'pilot_rehearsal:qt_{qi:06d}']['docs'])
   bi+=1; [p.requires_grad_(True) for p in model.parameters()]; x=torch.tensor([d['full_document_token_ids'] for d in docs],device=dev); q=torch.tensor([d['qdp'] for d in docs],device=dev); a=torch.tensor([d['answer_causal_position'] for d in docs],device=dev); y=torch.tensor([d['target_value_token'] for d in docs],device=dev); out,ex=model(x,q,a); ix=torch.arange(32,device=dev); la=F.cross_entropy(out[ix,a],y); li=loc_loss(ex['localization_attention'],docs); loss=la+LAM*li; scope='binding'
  else:
   [p.requires_grad_(True) for p in model.base_model.parameters()]; [p.requires_grad_(False) for p in model.localizer.parameters()]; [p.requires_grad_(False) for module in (model.wq,model.wk,model.wv,model.wo) for p in module.parameters()]; st=torch.randint(0,len(stream_tr)-257,size=(64,),generator=lg); off=torch.arange(256); x=stream_tr[st[:,None]+off[None,:]].to(dev); y=stream_tr[st[:,None]+off[None,:]+1].to(dev); out=model.base_model(x); loss=la=F.cross_entropy(out.reshape(-1,1024),y.reshape(-1)); li=torch.tensor(0.,device=dev); scope='language'
 ...
 model.eval(); final=OUT/'checkpoints/seed_8380/latest.pt'; final.parent.mkdir(parents=True,exist_ok=True); torch.save({'model_state_dict':model.state_dict(),'step':1000,'seed':SEED,'objective':'language_base_ce_alternated_with_t13_binding','language_updates':900,'binding_updates':100},final); post={'language_loss':lang_eval(model,stream_dv,dev,SEED+1001),'language_perplexity':math.exp(lang_eval(model,stream_dv,dev,SEED+1001)),'raw_generations':[generate(model,tok,p) for p in prompts],'binding':binding_eval(model,binddev,dev)}; (OUT/'training_metrics.jsonl').write_text(''.join(json.dumps(x)+'\n' for x in metrics),encoding='utf-8'); result={'status':'LANGUAGE_PILOT_0_COMPLETE','spec_sha256':spech,'starting_checkpoint_sha256':sha(CK),'final_checkpoint_sha256':sha(final),'tokenizer_sha256':sha(TOK),'language_train_sha256':sha(PILOT/'language_train.jsonl'),'language_dev_sha256':sha(PILOT/'language_dev.jsonl'),'binding_rehearsal_sha256':sha(PILOT/'binding_rehearsal.json'),'binding_dev_sha256':sha(PILOT/'binding_dev.json'),'updates':1000,'language_updates':900,'binding_updates':100,'optimizer_steps':1000,'sacred_retention_evaluations':0,'pre':pre,'post':post}; (OUT/'RESULTS.json').write_text(json.dumps(result,indent=2),encoding='utf-8'); (OUT/'REPORT.md').write_text('# Language Pilot 0 report\n\n'+json.dumps(result,indent=2),encoding='utf-8'); print(json.dumps(result,indent=2)); print('LANGUAGE_PILOT_0_COMPLETE')
