"""probe8: fast N-update mode probe replicating the frozen SF2 engine's update math.

Used ONLY for Path-1 reproducibility diagnosis. Does not checkpoint, does not
evaluate, does not modify any artifact. Prints a weight fingerprint after N
updates so that many short runs can be compared to detect discrete trajectory
modes across processes/environments.
"""
...
import SF2_ENGINE as E  # frozen engine module (reuse its exact functions)
from PINNED_MASKING import set_scope, pad_batch  # noqa

BUNDLE = Path(r"C:\DaveLM-CADAVER\single_fact_acquisition_sf1_seed87011")
N_UPDATES = int(os.environ.get("PROBE_N", "8"))
...
def main():
    ...
    fp = h.hexdigest()
    out = {
        "env": {k: v for k, v in os.environ.items() if k in ("TORCH_BLAS_PREFER_HIPBLASLT", "HIPBLASLT_AUTOTUNE", "PROBE_N")},
        "updates": N_UPDATES,
        "weight_fingerprint_sha256": fp,
        "metrics": metrics,
