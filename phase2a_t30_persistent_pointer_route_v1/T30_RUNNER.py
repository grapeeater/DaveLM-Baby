"""Phase 2A T30: T24 recipe plus a minimal persistent pointer-source route.
...
BUNDLE_DEFAULT = Path(r"C:\DaveLM-CADAVER\phase2a_t30_persistent_pointer_route_v1")
...
LAMBDA_FORK = 0.0
FORK_MARGIN = 1.0
AUTHORIZED_SEEDS = (870001, 870002, 870003)
CTX = 256
EOS = 3
...
def load_model():
    ...
    from T30_ROUTE import T30RoutedModel
    ...
    model = T30RoutedModel(BabyVNextConfig.load(ARCH / "BABY_VNEXT_CONFIG.json"))
    missing, unexpected = model.load_state_dict(payload["model_state_dict"], strict=False)
    if unexpected or missing != ["route_gate"]:
        raise RuntimeError(f"T30 parent schema mismatch missing={missing} unexpected={unexpected}")
    with torch.no_grad():
        model.route_gate.zero_()
...
def pointer_and_margin(model, row, device, train_mode: bool):
    ...
    for cand in row["candidate_token_ids"]:
        ...
        _, _, rr = model.pointer_route(hh, spans, p_len - 1)
        logits = model.logits_with_route(hh, rr, p_len - 1)
...
def greedy_answer(model, row, device):
    ...
    for _ in range(len(target) + 2):
        ...
        _, _, retrieved = model.pointer_route(hidden, spans, p_len - 1)
        logits = model.logits_with_route(hidden, retrieved, p_len - 1)
        t = int(logits[0, -1].argmax())
        gen.append(t)
        out_ids.append(t)
        if t == EOS:
...
def classify(rec: dict, u0_ce: float) -> str:
    ...
    if rep and max(sc.values()) >= (ptr / rec["dev"]["n"]) - 0.02:
        return "T30_SHORTCUT_FAILURE"
    nat = (rec["dev"]["native_forced_choice"] >= 96
           and rec["dev_reversals"]["native"] >= 48
           and rec["dev"]["exact_answer_eos"] >= 80)
    acq = rec["acq16_pass"]
    if not lang_ok:
        return "T30_LANGUAGE_REGRESSION"
    if not rep:
        return "T30_FAIL_NO_REPRESENTATION"
    if not nat or not acq:
        return "T30_REPRESENTATION_SUCCESS_OUTPUT_FAIL"
    return "T30_FULL_SUCCESS"
...
def main():
    ...
    model.to(device)
    binding_names = [n for n in model.state_dict() if not n.startswith("base_model.") and n != "route_gate"]
    ...
    def run_full(update: int) -> dict:
        ...
        rec = {
            ...
            "scope": scope,
            "route_gate_l2": float(torch.linalg.vector_norm(model.route_gate.detach()).cpu()),
            "route_gate_mean_abs": float(model.route_gate.detach().abs().mean().cpu()),
            "test_loaded": False,
        }
    ...
    def answer_span_ce_and_unl(row):
        ...
        ids = prompt + target
        if len(ids) > CTX + 1:
            raise RuntimeError(f"T30 sequence exceeds frozen context for {row.get('id')}")
        x = torch.tensor([ids[:-1]], device=device)
        y = torch.tensor([ids[1:]], device=device)
        hidden = model.base_model.forward_hidden(x)
        spans = fact_clause_spans_bos(row)
        logits, _, _, _ = model.route_logits_for_row(hidden, row, spans, p_len, p_len - 1)
        n = len(target)
    ...
    if args.mode == "smoke":
        ...
        payload = {
            ...
            "extra_forward_policy": "persistent route reuses base hidden; no generated-prefix rollout",
            "route_parameters": int(model.route_gate.numel()),
        }
    ...
    for update in range(resume_from + 1, TOTAL_UPDATES + 1):
        ...
        bad = [n for n, p in model.named_parameters()
               if p.grad is not None and (freeze_early(n, phase) or (not n.startswith("base_model.") and n != "route_gate"))]
        if bad:
        ...
        atomic_torch(ckdir / f"checkpoint_{update:04d}.pt", {
            "schema": "baby_vnext_phase2a_t30_persistent_pointer_route_lr3p75e5_1k_model_v1",
            "update": update,
            "seed": args.seed,
            "phase": phase,
            "model_state_dict": model.state_dict(),
        ...
        if rec["hard_stop"] or rec["dev_ce"] > 1.50:
            final_class, stop_code = "T30_HARD_STOP", "integrity_or_catastrophe"
            break
        if rec["dev_ce"] > 1.30:
            final_class, stop_code = "T30_LANGUAGE_REGRESSION", "language_ce_gt_1_30"
            break
