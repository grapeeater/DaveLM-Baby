"""Detached T21 sequential watchdog. Never loads T3 TEST."""
...
RUNNER = Path(r"C:\DaveLM-CADAVER\phase2a_t21_hybrid_answerce_lr3p75e5_1k_v1\T21_RUNNER.py")
BUNDLE = Path(r"C:\DaveLM-CADAVER\phase2a_t21_hybrid_answerce_lr3p75e5_1k_v1")
ROOT = Path(r"C:\DaveLM-CADAVER")
SEEDS = (800001, 800002, 800003)
HEARTBEAT = BUNDLE / "WATCHDOG_HEARTBEAT.json"
LOG = ROOT / "phase2a_t21_hybrid_answerce_lr3p75e5_1k_v1_watchdog.log"
POLL = 20
STUDY = "BABY_VNEXT_PHASE2A_T21_HYBRID_ANSWERCE_LR3P75E5_1K_V1"
...
def live_cmds():
    ...
    return [ln.strip() for ln in raw.splitlines()
            if "T21_RUNNER.py" in ln or ("t21_hybrid_answerce" in ln.lower() and "WATCHDOG" in ln)]


def out_dir(seed):
    return ROOT / f"phase2a_t21_hybrid_answerce_lr3p75e5_1k_v1_run_seed{seed}"
...
def owner_live():
    return any("T21_RUNNER.py" in c for c in live_cmds())


def run_seed(seed, resume):
    out = out_dir(seed)
    log_path = ROOT / f"phase2a_t21_hybrid_answerce_lr3p75e5_1k_v1_train_seed{seed}.log"
    env = os.environ.copy()
...
def classify_study(results):
    ...
    if any(c == "T21_HARD_STOP" for c in classes):
        return "T21_HARD_STOP"
    if classes.count("T21_FULL_SUCCESS") >= 2:
        return "T21_FULL_SUCCESS"
    if sum(c in ("T21_FULL_SUCCESS", "T21_REPRESENTATION_SUCCESS_OUTPUT_FAIL") for c in classes) >= 2:
        return "T21_REPRESENTATION_SUCCESS_OUTPUT_FAIL"
    if classes.count("T21_LANGUAGE_REGRESSION") >= 2:
        return "T21_LANGUAGE_REGRESSION"
    if classes.count("T21_SHORTCUT_FAILURE") >= 2:
...
def main():
    ...
    write_json(BUNDLE / "RUN_LEDGER.json", {
        ...
        "lr": 3.75e-5,
        "lambda_ans": 1.0,
        "answer_ce": "hybrid_first_through_base_suffix_stopgrad",
        "total_updates": 1000,
    })
    while True:
