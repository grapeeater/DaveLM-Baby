"""Phase 2A T14: T9 fact-clause pointer, 9:1, lr 3.75e-5, 750 updates.
...
BUNDLE_DEFAULT = Path(r"C:\DaveLM-CADAVER\phase2a_t14_factclause_pointer_lr3p75e5_750_v1")
...
def main():
    ...
    if args.seed not in (730001, 730002, 730003):
    ...
    for update in range(resume_from + 1, 751):
        ...
        atomic_torch(ckdir / f"checkpoint_{update:04d}.pt", {
            "schema": "baby_vnext_phase2a_t14_factclause_pointer_lr3p75e5_750_model_v1",
            "update": update,
            "seed": args.seed,
            "model_state_dict": model.state_dict(),
        })
        if rec["hard_stop"] or rec["dev_ce"] > 1.50:
            final_class, stop_code = "T14_HARD_STOP", "integrity_or_catastrophe"
            break
        if rec["dev_ce"] > 1.30:
            final_class, stop_code = "T14_LANGUAGE_REGRESSION", "language_ce_gt_1_30"
            break
        if update == 750:
