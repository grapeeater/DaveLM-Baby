"""Phase 2A T20: T18 two-phase recipe plus readout-only answer-span CE.

Answer CE stop-grads through the base: only language_head is trained by the CE term.
Pointer still trains late-base. Uniform lambda_ans=1.0 (no T19 suffix weight).
Never loads T3 TEST.
"""
...
BUNDLE_DEFAULT = Path(r"C:\DaveLM-CADAVER\phase2a_t20_readout_answerce_lr3p75e5_1k_v1")
...
def main():
    ...
    if args.seed not in (790001, 790002, 790003):
    ...
    def answer_span_ce(row):
        ...
        logits = model.base_model.language_head(hidden.detach())
        n = len(target)
        return F.cross_entropy(logits[0, -n:, :], y[0, -n:])
    ...
    for update in range(resume_from + 1, TOTAL_UPDATES + 1):
        ...
        atomic_torch(ckdir / f"checkpoint_{update:04d}.pt", {
            "schema": "baby_vnext_phase2a_t20_readout_answerce_pointer_lr3p75e5_1k_model_v1",
            "update": update,
            "seed": args.seed,
            "phase": phase,
            "model_state_dict": model.state_dict(),
        ...
        if rec["hard_stop"] or rec["dev_ce"] > 1.50:
            final_class, stop_code = "T20_HARD_STOP", "integrity_or_catastrophe"
            break
        if rec["dev_ce"] > 1.30:
            final_class, stop_code = "T20_LANGUAGE_REGRESSION", "language_ce_gt_1_30"
            break
