"""Detached T20 sequential watchdog. Never loads T3 TEST."""
...
RUNNER = Path(r"C:\DaveLM-CADAVER\phase2a_t20_readout_answerce_lr3p75e5_1k_v1\T20_RUNNER.py")
BUNDLE = Path(r"C:\DaveLM-CADAVER\phase2a_t20_readout_answerce_lr3p75e5_1k_v1")
ROOT = Path(r"C:\DaveLM-CADAVER")
SEEDS = (790001, 790002, 790003)
HEARTBEAT = BUNDLE / "WATCHDOG_HEARTBEAT.json"
LOG = ROOT / "phase2a_t20_readout_answerce_lr3p75e5_1k_v1_watchdog.log"
POLL = 20
STUDY = "BABY_VNEXT_PHASE2A_T20_READOUT_ANSWERCE_LR3P75E5_1K_V1"
...
def live_cmds():
    ...
    return [ln.strip() for ln in raw.splitlines()
            if "T20_RUNNER.py" in ln or ("t20_readout_answerce" in ln.lower() and "WATCHDOG" in ln)]


def out_dir(seed):
    return ROOT / f"phase2a_t20_readout_answerce_lr3p75e5_1k_v1_run_seed{seed}"
...
def owner_live():
    return any("T20_RUNNER.py" in c for c in live_cmds())


def run_seed(seed, resume):
    out = out_dir(seed)
    log_path = ROOT / f"phase2a_t20_readout_answerce_lr3p75e5_1k_v1_train_seed{seed}.log"
    env = os.environ.copy()
...
def classify_study(results):
    ...
    if any(c == "T20_HARD_STOP" for c in classes):
        return "T20_HARD_STOP"
    if classes.count("T20_FULL_SUCCESS") >= 2:
        return "T20_FULL_SUCCESS"
    if sum(c in ("T20_FULL_SUCCESS", "T20_REPRESENTATION_SUCCESS_OUTPUT_FAIL") for c in classes) >= 2:
        return "T20_REPRESENTATION_SUCCESS_OUTPUT_FAIL"
    if classes.count("T20_LANGUAGE_REGRESSION") >= 2:
        return "T20_LANGUAGE_REGRESSION"
    if classes.count("T20_SHORTCUT_FAILURE") >= 2:
        return "T20_SHORTCUT_FAILURE"
    return "T20_FAIL_NO_REPRESENTATION"
...
def main():
    ...
    write_json(BUNDLE / "RUN_LEDGER.json", {
        ...
        "lr": 3.75e-5,
        "lambda_ans": 1.0,
        "answer_ce": "readout_only_stopgrad_base",
        "total_updates": 1000,
    })
    while True:
