"""Phase 2A T13: T12 recipe, 9:1, lr 2.5e-5, 1000 updates.
...
BUNDLE_DEFAULT = Path(r"C:\DaveLM-CADAVER\phase2a_t13_factclause_pointer_lr2p5e5_1k_v1")
...
def main():
    ...
    if args.seed not in (720001, 720002, 720003):
    ...
    for update in range(resume_from + 1, 1001):
        kind = kinds[update - 1]
        model.train()
        ...
        atomic_torch(ckdir / f"checkpoint_{update:04d}.pt", {
            "schema": "baby_vnext_phase2a_t13_factclause_pointer_lr2p5e5_1k_model_v1",
            "update": update,
            "seed": args.seed,
            "model_state_dict": model.state_dict(),
        })
        if rec["hard_stop"] or rec["dev_ce"] > 1.50:
            final_class, stop_code = "T13_HARD_STOP", "integrity_or_catastrophe"
            break
        if rec["dev_ce"] > 1.30:
            final_class, stop_code = "T13_LANGUAGE_REGRESSION", "language_ce_gt_1_30"
            break
        if update == 1000:
