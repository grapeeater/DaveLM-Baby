"""SF10 independent continuations: SAME SF9 curriculum content, SAME lambda=0.25/M=1.0 margin
recipe, SAME KL/binding/scope/optimizer. ONLY change: each English update trains a deterministic
36-item balanced shard (1/4 of the 144-item curriculum) instead of all 144 items, cycled across
updates. Parents are fresh SF8 lambda=0.25 checkpoints (not the D3-polluted SF9 checkpoints).
"""
...
def verify(sealed=True):
    if sealed:
        ...
        assert receipt['status'] == 'SF10_PROSPECTIVE_PREFLIGHT_PASS'
...
def load_inputs():
    ...
    assert ei == 180
    assert set(exposure.values()) == {45} and len(exposure) == 144
...
def main():
    ...
    if a.mode == 'preload':
        print('SF10_PRE_PARENT_LOAD_PASS', a.seed, flush=True); return
