"""P11 gen-only aux overwrite versus matched λ=0.

Protocol: `design/V010_SELECTION_REPAIR_P11_GEN_ONLY.md`.
"""
...
from .selection_p10 import denials as p10_denials
...
OUT = ROOT / "runs/selection_p11"
PROTOCOL = ROOT / "design/V010_SELECTION_REPAIR_P11_GEN_ONLY.md"
S2_DIAGNOSTIC = ROOT / "runs/selection_s2/DIAGNOSTIC.json"
P10_ADJUDICATION = ROOT / "runs/selection_p10/ADJUDICATION_240001_400.json"
P10_SCHEDULES = (
    ROOT / "runs/selection_p10/SCHEDULE_240001.json",
    ROOT / "runs/selection_p10/SCHEDULE_240002.json",
)
...
GEN_ONLY = True
P10_LICENSE_VERDICT = "REGRESSION"
MAX_UPDATES = 800
KEYED_PER_BATCH = 8
...
def assert_p10_licenses_p11() -> dict:
    adj = json.loads(P10_ADJUDICATION.read_text(encoding="utf-8"))
    if adj.get("protocol") != "V010_SELECTION_REPAIR_P10_AUX_ONLY":
        raise RuntimeError("P11 requires P10 adjudication")
    if adj.get("verdict") != P10_LICENSE_VERDICT:
        raise RuntimeError("P11 is licensed only by P10 REGRESSION")
    return adj


def denials() -> tuple[set[tuple], set[tuple]]:
    denied_inputs, denied_spans = p10_denials()
    for path in P10_SCHEDULES:
        if not path.exists():
            raise RuntimeError(f"missing P10 schedule {path}")
...
def generate() -> None:
    ...
    p10 = assert_p10_licenses_p11()
    if (OUT / "MANIFEST.json").exists():
        raise RuntimeError("P11 already frozen")
    ...
    write(
        OUT / "MANIFEST.json",
        {
            "protocol": "V010_SELECTION_REPAIR_P11_GEN_ONLY",
            ...
            "gen_only": GEN_ONLY,
            "licensed_by": "V010_SELECTION_REPAIR_P10_AUX_ONLY",
            "p10_verdict": p10["verdict"],
...
def run(arm: str, seed: int, until: int) -> None:
    ...
    overwrite = LocalSlotOverwrite(config.d_model, gate_bias=GATE_BIAS, gen_only=GEN_ONLY).to(
    ...
    selection_s1.PACK_HOOK = lambda items, x, module=overwrite: module.set_gen_index(items, x.device)
    try:
        baseline = measure(model, overwrite, dev_items, diagnostic)
        write(dest / "eval_0000.json", baseline)
        debug_log(
            "P11",
            "selection_p11.py:run",
            "baseline",
            {
                "arm": arm,
