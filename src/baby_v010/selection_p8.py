"""P8 local slot scorer overwrite versus matched λ=0.

Protocol: `design/V010_SELECTION_REPAIR_P8_SLOT_OVERWRITE.md`.
"""
...
from .selection_p7 import denials as p7_denials
...
OUT = ROOT / "runs/selection_p8"
PROTOCOL = ROOT / "design/V010_SELECTION_REPAIR_P8_SLOT_OVERWRITE.md"
S2_DIAGNOSTIC = ROOT / "runs/selection_s2/DIAGNOSTIC.json"
P7_ADJUDICATION = ROOT / "runs/selection_p7/ADJUDICATION_210001_200.json"
P7_SCHEDULES = (
    ROOT / "runs/selection_p7/SCHEDULE_210001.json",
    ROOT / "runs/selection_p7/SCHEDULE_210002.json",
)

TRAIN_SEED = 220001
DATA_SEED = 220100
BOOTSTRAP_SEED = 220300
MAX_UPDATES = 800
KEYED_PER_BATCH = 8
...
def assert_p7_licenses_p8() -> dict:
    adj = json.loads(P7_ADJUDICATION.read_text(encoding="utf-8"))
    if adj.get("protocol") != "V010_SELECTION_REPAIR_P7_OVERWRITE_ONLY":
        raise RuntimeError("P8 requires P7 adjudication")
    if adj.get("verdict") != "NULL":
        raise RuntimeError("P8 is licensed only by P7 NULL")
    return adj


def denials() -> tuple[set[tuple], set[tuple]]:
    denied_inputs, denied_spans = p7_denials()
    for path in P7_SCHEDULES:
        if not path.exists():
            raise RuntimeError(f"missing P7 schedule {path}")
...
def generate() -> None:
    ...
    p7 = assert_p7_licenses_p8()
    if (OUT / "MANIFEST.json").exists():
        raise RuntimeError("P8 already frozen")
    ...
    write(
        OUT / "MANIFEST.json",
        {
            "protocol": "V010_SELECTION_REPAIR_P8_SLOT_OVERWRITE",
            "parent_sha256": PARENT_SHA,
            "ptr_lambda_treatment": PTR_LAMBDA,
            "gate_lambda_treatment_after_200": GATE_LAMBDA,
            "lambda_control": 0.0,
            "licensed_by": "V010_SELECTION_REPAIR_P7_OVERWRITE_ONLY",
