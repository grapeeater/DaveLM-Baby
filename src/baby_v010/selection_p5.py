"""P5 identity residual overwrite versus matched λ=0.

Protocol: `design/V010_SELECTION_REPAIR_P5_OVERWRITE.md`.
"""
...
from .selection_p4 import copy_specs, denials as p4_denials
...
OUT = ROOT / "runs/selection_p5"
PROTOCOL = ROOT / "design/V010_SELECTION_REPAIR_P5_OVERWRITE.md"
S2_DIAGNOSTIC = ROOT / "runs/selection_s2/DIAGNOSTIC.json"
D4_ADJUDICATION = ROOT / "runs/query_attn_patch_d4/ADJUDICATION.json"
P4_SCHEDULES = (
    ROOT / "runs/selection_p4/SCHEDULE_180001.json",
    ROOT / "runs/selection_p4/SCHEDULE_180002.json",
)

TRAIN_SEED = 190001
DATA_SEED = 190100
BOOTSTRAP_SEED = 190300
MAX_UPDATES = 800
KEYED_PER_BATCH = 8
...
def assert_d4_licenses_p5() -> dict:
    adj = json.loads(D4_ADJUDICATION.read_text(encoding="utf-8"))
    if adj.get("protocol") != "V010_QUERY_ATTN_PATCH_D4":
        raise RuntimeError("P5 requires D4 adjudication")
    if adj.get("verdict") != "OV_DEAD":
        raise RuntimeError("P5 is licensed only by D4 OV_DEAD")
    return adj


def denials() -> tuple[set[tuple], set[tuple]]:
    denied_inputs, denied_spans = p4_denials()
    for path in P4_SCHEDULES:
        if not path.exists():
            raise RuntimeError(f"missing P4 schedule {path}")
...
def generate() -> None:
    ...
    d4 = assert_d4_licenses_p5()
    if (OUT / "MANIFEST.json").exists():
        raise RuntimeError("P5 already frozen")
    ...
    write(
        OUT / "MANIFEST.json",
        {
            "protocol": "V010_SELECTION_REPAIR_P5_OVERWRITE",
            "parent_sha256": PARENT_SHA,
            "ptr_lambda_treatment": PTR_LAMBDA,
            "gate_lambda_treatment_after_200": GATE_LAMBDA,
            "lambda_control": 0.0,
            "licensed_by": "V010_QUERY_ATTN_PATCH_D4",
...
def run(arm: str, seed: int, until: int) -> None:
    ...
    overwrite = IdentityResidualOverwrite(config.d_model, gate_bias=GATE_BIAS).to(next(model.parameters()).device)
