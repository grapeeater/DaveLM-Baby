"""P7 frozen-Baby identity overwrite at 1e-3 versus matched λ=0.

Protocol: `design/V010_SELECTION_REPAIR_P7_OVERWRITE_ONLY.md`.
"""
...
from .selection_p6 import denials as p6_denials
...
OUT = ROOT / "runs/selection_p7"
PROTOCOL = ROOT / "design/V010_SELECTION_REPAIR_P7_OVERWRITE_ONLY.md"
S2_DIAGNOSTIC = ROOT / "runs/selection_s2/DIAGNOSTIC.json"
P6_ADJUDICATION = ROOT / "runs/selection_p6/ADJUDICATION_200001_200.json"
P6_SCHEDULES = (
    ROOT / "runs/selection_p6/SCHEDULE_200001.json",
    ROOT / "runs/selection_p6/SCHEDULE_200002.json",
)

TRAIN_SEED = 210001
DATA_SEED = 210100
BOOTSTRAP_SEED = 210300
MAX_UPDATES = 800
KEYED_PER_BATCH = 8
...
def assert_p6_licenses_p7() -> dict:
    adj = json.loads(P6_ADJUDICATION.read_text(encoding="utf-8"))
    if adj.get("protocol") != "V010_SELECTION_REPAIR_P6_OVERWRITE_XAVIER":
        raise RuntimeError("P7 requires P6 adjudication")
    if adj.get("verdict") != "REGRESSION":
        raise RuntimeError("P7 is licensed only by P6 REGRESSION")
    return adj


def denials() -> tuple[set[tuple], set[tuple]]:
    denied_inputs, denied_spans = p6_denials()
    for path in P6_SCHEDULES:
        if not path.exists():
            raise RuntimeError(f"missing P6 schedule {path}")
...
def generate() -> None:
    ...
    p6 = assert_p6_licenses_p7()
    if (OUT / "MANIFEST.json").exists():
        raise RuntimeError("P7 already frozen")
    ...
    write(
        OUT / "MANIFEST.json",
        {
            "protocol": "V010_SELECTION_REPAIR_P7_OVERWRITE_ONLY",
            "parent_sha256": PARENT_SHA,
            "ptr_lambda_treatment": PTR_LAMBDA,
            "gate_lambda_treatment_after_200": GATE_LAMBDA,
            "lambda_control": 0.0,
            "licensed_by": "V010_SELECTION_REPAIR_P6_OVERWRITE_XAVIER",
            "p6_verdict": p6["verdict"],
            "qk_init": "xavier",
