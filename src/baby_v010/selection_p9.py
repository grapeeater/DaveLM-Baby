"""P9 slot overwrite through gate-on versus matched λ=0.

Protocol: `design/V010_SELECTION_REPAIR_P9_SLOT_GATEON.md`.
"""
...
from .selection_p8 import denials as p8_denials
...
OUT = ROOT / "runs/selection_p9"
PROTOCOL = ROOT / "design/V010_SELECTION_REPAIR_P9_SLOT_GATEON.md"
S2_DIAGNOSTIC = ROOT / "runs/selection_s2/DIAGNOSTIC.json"
P8_ADJUDICATION = ROOT / "runs/selection_p8/ADJUDICATION_220001_200.json"
P8_SCHEDULES = (
    ROOT / "runs/selection_p8/SCHEDULE_220001.json",
    ROOT / "runs/selection_p8/SCHEDULE_220002.json",
)

TRAIN_SEED = 230001
DATA_SEED = 230100
BOOTSTRAP_SEED = 230300
MAX_UPDATES = 800
KEYED_PER_BATCH = 8
...
def assert_p8_licenses_p9() -> dict:
    adj = json.loads(P8_ADJUDICATION.read_text(encoding="utf-8"))
    if adj.get("protocol") != "V010_SELECTION_REPAIR_P8_SLOT_OVERWRITE":
        raise RuntimeError("P9 requires P8 adjudication")
    if adj.get("verdict") != "NULL":
        raise RuntimeError("P9 is licensed only by P8 NULL")
    return adj


def denials() -> tuple[set[tuple], set[tuple]]:
    denied_inputs, denied_spans = p8_denials()
    for path in P8_SCHEDULES:
        if not path.exists():
            raise RuntimeError(f"missing P8 schedule {path}")
...
def generate() -> None:
    ...
    p8 = assert_p8_licenses_p9()
    if (OUT / "MANIFEST.json").exists():
        raise RuntimeError("P9 already frozen")
    ...
    write(
        OUT / "MANIFEST.json",
        {
            "protocol": "V010_SELECTION_REPAIR_P9_SLOT_GATEON",
            "parent_sha256": PARENT_SHA,
            "ptr_lambda_treatment": PTR_LAMBDA,
            "gate_lambda_treatment_after_200": GATE_LAMBDA,
            "lambda_control": 0.0,
            "licensed_by": "V010_SELECTION_REPAIR_P8_SLOT_OVERWRITE",
            "p8_verdict": p8["verdict"],
