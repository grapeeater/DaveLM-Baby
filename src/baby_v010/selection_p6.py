"""P6 identity residual overwrite with Xavier Q/K versus matched λ=0.

Protocol: `design/V010_SELECTION_REPAIR_P6_OVERWRITE_XAVIER.md`.
"""
...
from .residual_overwrite import IdentityResidualOverwrite, attach_overwrite, pointer_aux
...
from .selection_p5 import denials as p5_denials
...
from .train_v2r4 import HIGH_LR

ROOT = Path(__file__).resolve().parents[2]
OUT = ROOT / "runs/selection_p6"
PROTOCOL = ROOT / "design/V010_SELECTION_REPAIR_P6_OVERWRITE_XAVIER.md"
S2_DIAGNOSTIC = ROOT / "runs/selection_s2/DIAGNOSTIC.json"
P5_ADJUDICATION = ROOT / "runs/selection_p5/ADJUDICATION_190001_200.json"
P5_SCHEDULES = (
    ROOT / "runs/selection_p5/SCHEDULE_190001.json",
    ROOT / "runs/selection_p5/SCHEDULE_190002.json",
)

TRAIN_SEED = 200001
DATA_SEED = 200100
BOOTSTRAP_SEED = 200300
MAX_UPDATES = 800
KEYED_PER_BATCH = 8
...
def assert_p5_licenses_p6() -> dict:
    adj = json.loads(P5_ADJUDICATION.read_text(encoding="utf-8"))
    if adj.get("protocol") != "V010_SELECTION_REPAIR_P5_OVERWRITE":
        raise RuntimeError("P6 requires P5 adjudication")
    if adj.get("verdict") != "REGRESSION":
        raise RuntimeError("P6 is licensed only by P5 REGRESSION")
    return adj


def denials() -> tuple[set[tuple], set[tuple]]:
    denied_inputs, denied_spans = p5_denials()
    for path in P5_SCHEDULES:
        if not path.exists():
            raise RuntimeError(f"missing P5 schedule {path}")
...
def generate() -> None:
    ...
    p5 = assert_p5_licenses_p6()
    if (OUT / "MANIFEST.json").exists():
        raise RuntimeError("P6 already frozen")
    ...
    write(
        OUT / "MANIFEST.json",
        {
            "protocol": "V010_SELECTION_REPAIR_P6_OVERWRITE_XAVIER",
            "parent_sha256": PARENT_SHA,
            "ptr_lambda_treatment": PTR_LAMBDA,
            "gate_lambda_treatment_after_200": GATE_LAMBDA,
            "lambda_control": 0.0,
            "licensed_by": "V010_SELECTION_REPAIR_P5_OVERWRITE",
            "p5_verdict": p5["verdict"],
            "qk_init": "xavier",
...
def run(arm: str, seed: int, until: int) -> None:
    ...
    overwrite = IdentityResidualOverwrite(config.d_model, gate_bias=GATE_BIAS, qk_init="xavier").to(
        next(model.parameters()).device
    )
    optimizer, _, _ = capability_optimizer(model)
    optimizer.add_param_group({"params": list(overwrite.parameters()), "lr": HIGH_LR})
    ...
    try:
        ...
        for step in range(1, until + 1):
            ...
            if not torch.isfinite(loss):
                ...
                break
            loss.backward()
            params = list(model.parameters()) + list(overwrite.parameters())
            norm = torch.nn.utils.clip_grad_norm_(params, 2.0)
