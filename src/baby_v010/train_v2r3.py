...
def freeze_language_prefix(model) -> int:
    """Preserve the language-acquired prefix; train the T34 upper-block scope."""
    trainable = 0
    for name, parameter in model.named_parameters():
        allowed = (
            name.startswith("blocks.7.")
            or name.startswith("blocks.8.")
            or name.startswith("blocks.9.")
            or name.startswith("blocks.10.")
            or name.startswith("blocks.11.")
            or name.startswith("final_norm.")
            or name.startswith("language_head.")
        )
        parameter.requires_grad_(allowed)
        if allowed:
            trainable += parameter.numel()
    if trainable <= 0:
        raise RuntimeError("empty v2R3 capability scope")
    return trainable
def run(seed: int, out: Path, updates: int, eval_interval: int, device: torch.device, batch_size: int) -> None:
    ...
    (out / "RUN_CONFIG.json").write_text(json.dumps({"seed": seed, "updates": updates, "eval_interval": eval_interval, "device": str(device), "protocol": PROTOCOL, "parent_checkpoint": None, "protected_material_opened": False, "language_effective_batch": 64, "structured_learning_rate": 3.75e-5, "freeze_prefix_after_update": 6000}, indent=2) + "\n", encoding="utf-8")
    ...
    with metrics_path.open("w", encoding="utf-8") as metrics:
        for update in range(updates + 1):
            ...
            if update == updates:
                break
            if update == LANGUAGE_END:
                # A fresh optimizer marks the deliberate transition from
                # language acquisition to low-LR capability acquisition.
                trainable = freeze_language_prefix(model)
                optimizer = torch.optim.AdamW([p for p in model.parameters() if p.requires_grad], lr=3.75e-5, betas=(0.9, 0.999), eps=1e-8, weight_decay=0.05)
            model.train()
            ...
            if stage == "language_foundation":
                ...
                optimizer.step()
                task = "language_effective_batch_64"
