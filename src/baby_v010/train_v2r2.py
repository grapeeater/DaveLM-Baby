...
def run(seed: int, out: Path, updates: int, eval_interval: int, device: torch.device, batch_size: int) -> None:
    ...
    (out / "RUN_CONFIG.json").write_text(json.dumps({"seed": seed, "updates": updates, "eval_interval": eval_interval, "device": str(device), "protocol": PROTOCOL, "parent_checkpoint": None, "protected_material_opened": False, "language_effective_batch": 64, "structured_learning_rate": 3.75e-5}, indent=2) + "\n", encoding="utf-8")
    ...
    with metrics_path.open("w", encoding="utf-8") as metrics:
        for update in range(updates + 1):
            ...
            if update == LANGUAGE_END:
                ...
                optimizer = torch.optim.AdamW(model.parameters(), lr=3.75e-5, betas=(0.9, 0.999), eps=1e-8, weight_decay=0.05)
            model.train()
            ...
            if stage == "language_foundation":
                ...
                if stage == "full_foundation" and rng.random() < 0.20:
                    x, y = language_batch(train_stream, rng, batch_size, config.context_length, device)
                    logits = model(x)
