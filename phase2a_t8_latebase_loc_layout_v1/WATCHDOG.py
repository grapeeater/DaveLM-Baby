"""Detached T8 sequential watchdog. Never loads T3 TEST."""
...
RUNNER = Path(r"C:\DaveLM-CADAVER\phase2a_t8_latebase_loc_layout_v1\T8_RUNNER.py")
BUNDLE = Path(r"C:\DaveLM-CADAVER\phase2a_t8_latebase_loc_layout_v1")
ROOT = Path(r"C:\DaveLM-CADAVER")
SEEDS = (670001, 670002, 670003)
HEARTBEAT = BUNDLE / "WATCHDOG_HEARTBEAT.json"
LOG = ROOT / "phase2a_t8_latebase_loc_layout_v1_watchdog.log"
POLL = 20
...
def live_cmds():
    ...
    return [ln.strip() for ln in raw.splitlines()
            if "T8_RUNNER.py" in ln or ("t8_latebase" in ln.lower() and "WATCHDOG" in ln)]


def out_dir(seed):
    return ROOT / f"phase2a_t8_latebase_loc_layout_v1_run_seed{seed}"
...
def owner_live():
    return any("T8_RUNNER.py" in c for c in live_cmds())


def run_seed(seed, resume):
    out = out_dir(seed)
    log_path = ROOT / f"phase2a_t8_latebase_loc_layout_v1_train_seed{seed}.log"
    env = os.environ.copy()
...
def classify_study(results):
    ...
    if any(c == "T8_HARD_STOP" for c in classes):
        return "T8_HARD_STOP"
    if classes.count("T8_BINDING_SUCCESS") >= 2:
        return "T8_BINDING_SUCCESS"
    if sum(c in ("T8_BINDING_SUCCESS", "T8_ROUTING_SUCCESS_OUTPUT_FAIL") for c in classes) >= 2:
        return "T8_ROUTING_SUCCESS_OUTPUT_FAIL"
    if classes.count("T8_LANGUAGE_REGRESSION") >= 2:
...
def main():
    ...
    write_json(BUNDLE / "RUN_LEDGER.json", {
        ...
        "study": "BABY_VNEXT_PHASE2A_T8_LATEBASE_LOC_LAYOUT_V1",
        ...
        "objective": "loc CE + layout native margin; no T3 pointer",
    })
    while True:
