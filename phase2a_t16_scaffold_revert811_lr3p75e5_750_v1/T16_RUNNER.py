"""Phase 2A T16: two-phase scaffold. Phase A trains 4-11; after U500 revert/freeze 8-11 at Phase1G and train 4-7.
...
BUNDLE_DEFAULT = Path(r"C:\DaveLM-CADAVER\phase2a_t16_scaffold_revert811_lr3p75e5_750_v1")
...
def main():
    ...
    if args.seed not in (750001, 750002, 750003):
    ...
    for update in range(resume_from + 1, 751):
        ...
        atomic_torch(ckdir / f"checkpoint_{update:04d}.pt", {
            "schema": "baby_vnext_phase2a_t16_scaffold_revert811_pointer_lr3p75e5_750_model_v1",
            "update": update,
            "seed": args.seed,
            "phase": phase,
            "model_state_dict": model.state_dict(),
        ...
        if rec["hard_stop"] or rec["dev_ce"] > 1.50:
            final_class, stop_code = "T16_HARD_STOP", "integrity_or_catastrophe"
            break
        if rec["dev_ce"] > 1.30:
            final_class, stop_code = "T16_LANGUAGE_REGRESSION", "language_ce_gt_1_30"
            break
        if update == 750:
