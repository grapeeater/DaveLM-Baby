"""Detached T16 sequential watchdog. Never loads T3 TEST."""
...
RUNNER = Path(r"C:\DaveLM-CADAVER\phase2a_t16_scaffold_revert811_lr3p75e5_750_v1\T16_RUNNER.py")
BUNDLE = Path(r"C:\DaveLM-CADAVER\phase2a_t16_scaffold_revert811_lr3p75e5_750_v1")
ROOT = Path(r"C:\DaveLM-CADAVER")
SEEDS = (750001, 750002, 750003)
HEARTBEAT = BUNDLE / "WATCHDOG_HEARTBEAT.json"
LOG = ROOT / "phase2a_t16_scaffold_revert811_lr3p75e5_750_v1_watchdog.log"
POLL = 20
...
def live_cmds():
    ...
    return [ln.strip() for ln in raw.splitlines()
            if "T16_RUNNER.py" in ln or ("t16_scaffold_revert811" in ln.lower() and "WATCHDOG" in ln)]


def out_dir(seed):
    return ROOT / f"phase2a_t16_scaffold_revert811_lr3p75e5_750_v1_run_seed{seed}"
...
def owner_live():
    return any("T16_RUNNER.py" in c for c in live_cmds())


def run_seed(seed, resume):
    out = out_dir(seed)
    log_path = ROOT / f"phase2a_t16_scaffold_revert811_lr3p75e5_750_v1_train_seed{seed}.log"
    env = os.environ.copy()
...
def classify_study(results):
    ...
    if any(c == "T16_HARD_STOP" for c in classes):
        return "T16_HARD_STOP"
    if classes.count("T16_FULL_SUCCESS") >= 2:
        return "T16_FULL_SUCCESS"
    if sum(c in ("T16_FULL_SUCCESS", "T16_REPRESENTATION_SUCCESS_OUTPUT_FAIL") for c in classes) >= 2:
        return "T16_REPRESENTATION_SUCCESS_OUTPUT_FAIL"
    if classes.count("T16_LANGUAGE_REGRESSION") >= 2:
        return "T16_LANGUAGE_REGRESSION"
    if classes.count("T16_SHORTCUT_FAILURE") >= 2:
        return "T16_SHORTCUT_FAILURE"
    return "T16_FAIL_NO_REPRESENTATION"
...
def main():
    ...
    write_json(BUNDLE / "RUN_LEDGER.json", {
        ...
        "study": "BABY_VNEXT_PHASE2A_T16_SCAFFOLD_REVERT811_LR3P75E5_750_V1",
        ...
        "scope": {"phase_a_updates": "1-500", "phase_a_trainable": 40849664, "phase_b_after": 500, "phase_b_trainable": 21163264},
