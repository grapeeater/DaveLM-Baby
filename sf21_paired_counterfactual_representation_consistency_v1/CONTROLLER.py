"""SF21: Paired Counterfactual Representation Consistency.

ONE scientific variable vs SF20: add paired counterfactual representation consistency term.
Full CE, broad KL160, margin, optimizer, scope, schedule, gates, binding, evaluation, and persistence remain unchanged.
"""
...
def consistency_term(logits, ids_in_batch, idx):
    """Paired counterfactual representation consistency.
    
    For each record in the batch, extract the logits at position = prefix_len - 1
    (the last context token before answer generation, same position as margin hinge).
    Group records by pair_id to form positive pairs (same template, different identities).
    Compute cosine similarity between normalized logit vectors of paired records.
    Loss = mean(1 - cos_sim) over all valid pairs in the batch.
    
    Uses logits (pre-softmax) at the target position as the representation.
    This is a linear projection of the hidden state and preserves the relative
    geometry needed for the consistency objective.
    """
    # Build mapping: pair_id -> list of (batch_index, prefix_len, representation_vector)
    pair_groups = defaultdict(list)
    
    for k, rid in enumerate(ids_in_batch):
        r = idx[rid]
        prefix_len = 1 + len(r['prompt_token_ids'])
        # Extract logits at position prefix_len - 1 for this record
        rep = logits[k, prefix_len - 1]  # [1024]
        pair_groups[r['pair_id']].append((k, prefix_len, rep))
    
    # Filter to pairs with exactly 2 records (the standard SF20 pair structure)
    valid_pairs = []
    for pair_id, members in pair_groups.items():
        if len(members) == 2:
            # Both members present in this batch
            (k1, pl1, rep1), (k2, pl2, rep2) = members
            valid_pairs.append((rep1, rep2))
    
    if not valid_pairs:
        # No complete pairs in this batch; return zero loss
        return torch.tensor(0.0, device=logits.device, requires_grad=True)
    
    # L2 normalize representations
    reps = torch.stack([rep for pair in valid_pairs for rep in pair])
    reps_norm = F.normalize(reps, p=2, dim=-1)
    
    # Reshape to [n_pairs, 2, 1024]
    n_pairs = len(valid_pairs)
    reps_paired = reps_norm.reshape(n_pairs, 2, -1)
    
    # Compute cosine similarity: for each pair, cos_sim = (v1 • v2) / (|v1||v2|)
    # Since both are L2-normalized, cos_sim = v1 • v2
    cos_sims = torch.einsum('bik,bjk->bi', reps_paired, reps_paired).diagonal(dim1=-2, dim2=-1)
    # cos_sims shape: [n_pairs, 2] but we want the off-diagonal (pair similarity)
    # Actually, einsum('bik,bjk->bi') gives [n_pairs, 2] where each row is [v1•v1, v1•v2] for pair i
    # We want v1•v2 for each pair
    # Better: use bmm
    cos_sim = torch.bmm(reps_paired[:, 0:1, :], reps_paired[:, 1:, :].transpose(-2, -1)).squeeze(-1).squeeze(-1)
    
    # Consistency loss: 1 - cos_sim, minimized when representations are similar
...
def verify(sealed=True):
    if sealed:
        ...
        assert receipt['status'] == 'SF21_PROSPECTIVE_PREFLIGHT_PASS'
...
def main():
    ...
    if a.mode == 'preload':
        print('SF21_PRE_PARENT_LOAD_PASS', a.seed, flush=True); return
    ...
    provenance = {'parent_checkpoint': run['parent_checkpoint'], 'parent_checkpoint_sha256': run['parent_checkpoint_sha256'],
                  ...
                  'lambda_consistency': LAMBDA_CONSISTENCY, 'pair_manifest': sha(H / 'PAIR_MANIFEST.json')}
    ...
    if completed == 0:
        ...
        # Parent-specific U0 reproduction: each SF21 branch must reproduce its OWN SF8 parent.
    ...
    for u in schedule[completed:]:
        ...
        ce = kl_val = margin_loss = consistency_loss = None
        if u['kind'] == 'english':
            english_index += 1
            m.eval()
            ...
            # SF21: Add paired counterfactual representation consistency term
            consistency_loss = consistency_term(logits, u['ids'], idx)
        ...
        if ce is not None: rec.update(ce=float(ce.detach()), kl=float(kl_val.detach()), shard=u.get('shard'))
        if margin_loss is not None: rec.update(margin_loss=float(margin_loss.detach()))
        if consistency_loss is not None: rec.update(consistency_loss=float(consistency_loss.detach()))
