"""Phase 2A T23: T18 two-phase recipe plus in-row candidate unlikelihood.

Full-span answer CE through the base (T18). Plus mean -log(1-p) on in-row
wrong-candidate tokens at the same answer offset. lambda_ans=lambda_unl=1.0.
Never loads T3 TEST.
"""
...
BUNDLE_DEFAULT = Path(r"C:\DaveLM-CADAVER\phase2a_t23_candidate_unlikelihood_lr3p75e5_1k_v1")
...
def main():
    ...
    if args.seed not in (820001, 820002, 820003):
    ...
    def answer_span_ce_and_unl(row):
        ...
        rivals = [[int(t) for t in c] + [EOS]
                  for i, c in enumerate(row["candidate_token_ids"]) if i != ci]
        ids = prompt + target
        if len(ids) > CTX + 1:
            ids = ids[-(CTX + 1):]
    ...
    for update in range(resume_from + 1, TOTAL_UPDATES + 1):
        ...
        atomic_torch(ckdir / f"checkpoint_{update:04d}.pt", {
            "schema": "baby_vnext_phase2a_t23_candidate_unlikelihood_pointer_lr3p75e5_1k_model_v1",
            "update": update,
            "seed": args.seed,
            "phase": phase,
            "model_state_dict": model.state_dict(),
        ...
        if rec["hard_stop"] or rec["dev_ce"] > 1.50:
            final_class, stop_code = "T23_HARD_STOP", "integrity_or_catastrophe"
            break
        if rec["dev_ce"] > 1.30:
            final_class, stop_code = "T23_LANGUAGE_REGRESSION", "language_ce_gt_1_30"
            break
