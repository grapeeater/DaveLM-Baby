...
def verify(sealed=True):
    if sealed:
        ...
        assert receipt['status'] == 'SF12_PROSPECTIVE_PREFLIGHT_PASS'
...
def main():
    ...
    if a.mode == 'preload':
        print('SF12_PRE_PARENT_LOAD_PASS', a.seed, flush=True); return
    ...
    for u in schedule[completed:]:
        ...
        ce = kl_val = margin_loss = name_val = name_telemetry = None
        if u['kind'] == 'english':
            english_index += 1
            m.eval()
        ...
        if margin_loss is not None: rec.update(margin_loss=float(margin_loss.detach()), name_retention=name_telemetry)
