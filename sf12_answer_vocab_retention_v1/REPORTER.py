...
def summarize(seed):
    ...
    return {'seed':seed,'status':st['status'],'completed':u,'checkpoint':st['checkpoint'],'checkpoint_sha256':st['checkpoint_sha256'],'gates':st['gates'],
      ...
      'binding':ch['binding'],'R_name_endpoint_probe':tel,'R_name_training_mean':train_tel}
...
def render(result):
    ...
    for r in result['runs']:
      b='; '.join(f"{k}:{v['answer']}/{v['both_distinct']}/c{v['collapse']}" for k,v in r['binding'].items())
    ...
    lines += ['', '# DAVE-CODED INTERPRETATION','',result['dave_coded'],'', '# SCIENTIFIC INTERPRETATION','',result['scientific_interpretation'],'', '# ARTIFACTS','',f"- Study: `{H}`",f"- Machine-readable results: `{H/'RESULTS.json'}`",f"- Output hashes: `{H/'OUTPUT_SHA256SUMS.txt'}`",'', '# EXACTLY ONE NEXT ACTION','',result['next_action'],'']
    return '\n'.join(lines)
...
def main():
    ...
    C.rt.atomic_json(result,H/'RESULTS.json'); (H/'FINAL_REPORT.md').write_text(render(result),encoding='utf-8',newline='\n')
    payload=[]
