"""Independent full-trajectory SF7 runs. Pinned SF2/SF6 objectives/evaluators/schedule/scope/
optimizer/gates, unchanged. ONLY new element: a pairwise first-answer-token margin hinge added
to the English factual loss in the treatment arm only. No LR schedule change (constant 5e-5 in
both arms, no annealing). No data-balance change. No EOS-loss change. No candidate-string change.
"""
...
def main():
    ...
    if a.mode == 'preload':
        print('SF7_PRE_PARENT_LOAD_PASS', a.seed, a.arm, flush=True); return
    ...
    provenance = {'parent': p['parent_sha256'], 'protocol': sha(H / 'PROTOCOL.json'),
                  ...
                  'margin_M': MARGIN_M, 'lambda_margin': LAMBDA_MARGIN}
    ...
    for u in schedule[completed:]:
        ...
        for group in opt.param_groups: group['lr'] = 5e-5  # constant in BOTH arms; no annealing in SF7
